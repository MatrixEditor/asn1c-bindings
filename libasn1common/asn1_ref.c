...
void asn1p_ref_free(asn1p_ref_t *ref) {
    if (ref) {
        if (ref->components) {
...
void asn1p_ref_set_source(asn1p_ref_t *ref, struct asn1p_module_s *module,
                          int lineno) {
    if (ref) {
        ref->module = module;
        ref->_lineno = lineno;
...
static enum asn1p_ref_lex_type_e asn1p_ref_name2lextype(const char *name) {
...
int asn1p_ref_add_component(asn1p_ref_t *ref, const char *name,
                            enum asn1p_ref_lex_type_e lex_type) {
    if (!ref || !name || (int)lex_type < RLT_UNKNOWN || lex_type >= RLT_MAX) {
        errno = EINVAL;
        return -1;
    }
    ...
    if (ref->comp_count == ref->comp_size) {
        ...
        void *p =
            realloc(ref->components, newsize * sizeof(ref->components[0]));
        if (p) {
            ref->components = p;
...
asn1p_ref_t *asn1p_ref_clone(asn1p_ref_t *ref) {
    ...
    if (newref) {
        for (size_t i = 0; i < ref->comp_count; i++) {
            if (asn1p_ref_add_component(newref, ref->components[i].name,
...
int asn1p_ref_compare(const asn1p_ref_t *a, const asn1p_ref_t *b) {
    if (a->comp_count != b->comp_count) return -1;
...
const char *asn1p_ref_string(const asn1p_ref_t *ref) {
