...
static int ulong_optimization(arg_t *arg, asn1p_expr_type_e etype,
...
int asn1c_emit_constraint_checking_code(arg_t *arg) {
    ...
    if (ct == NULL) return 1; /* No additional constraints defined */

    etype = _find_terminal_type(arg);

    r_value = asn1constraint_compute_constraint_range(
        expr->Identifier, etype, ct, ACT_EL_RANGE, 0, 0, 0);
    r_size = asn1constraint_compute_constraint_range(expr->Identifier, etype,
                                                     ct, ACT_CT_SIZE, 0, 0, 0);
    if (r_value) {
        if (r_value->incompatible || r_value->empty_constraint ||
            (r_value->left.type == ARE_MIN && r_value->right.type == ARE_MAX) ||
            (etype == ASN_BASIC_BOOLEAN && r_value->left.value == 0 &&
    ...
    if (r_size) {
        if (r_size->incompatible || r_size->empty_constraint ||
    ...
    switch (etype) {
        ...
        case ASN_BASIC_OCTET_STRING:
            ...
            break;
        default:
            if (etype & ASN_STRING_MASK) produce_st = 1;
            break;
    }
    if (produce_st) {
    ...
    if (r_size || r_value) {
        ...
        if (r_value) switch (etype) {
                case ASN_BASIC_INTEGER:
                case ASN_BASIC_ENUMERATED:
                    if (native_long_sign(arg, r_value) >= 0) {
                        ulong_optimize =
                            ulong_optimization(arg, etype, r_size, r_value);
                        if (!ulong_optimize) {
                            value_unsigned = 1;
    ...
    if (r_size) emit_size_determination_code(arg, etype);
    ...
    if (alphabet_table_compiled) {
        ...
        OUT("!check_permitted_alphabet_%d(%s)", arg->expr->_type_unique_index,
            produce_st ? "st" : "sptr");
        got_something++;
    }
    if (!got_something) {
    ...
    switch (etype) {
        case ASN_CONSTR_SEQUENCE_OF:
            ...
            OUT("return SEQUENCE_OF_constraint(td, sptr, ctfailcb, "
                "app_key);\n");
            break;
...
static int asn1c_emit_constraint_tables(arg_t *arg, int got_size) {
    ...
    range = asn1constraint_compute_constraint_range(
        arg->expr->Identifier, etype, ct, ACT_CT_FROM, 0, 0, 0);
    if (!range) return 0;
    ...
    if ((range_stop - range_start) > 255) use_table = 0;
    ...
    switch (etype) {
        ...
        default:
            if (etype & ASN_STRING_MASK) {
                ...
                WARNING("SizeConstraint is not defined for %s", type_name);
                OUT_NOINDENT(
                    "#warning SizeConstraint "
    ...
    switch (etype) {
        ...
        case ASN_BASIC_ENUMERATED:
            if (asn1c_type_fits_long(arg, arg->expr) == FL_FITS_UNSIGN) {
                ...
                if (r_value->el_count == 0 &&
                    ...
                    OUT("value = st->buf ? ((st->buf[0] & 0x80) ? -1 : 1) : "
                        "0;\n");
                    break;
                }
        ...
        default:
            WARNING(
                ...
                arg->expr->module->source_file_name, arg->expr->_lineno,
                arg->expr->Identifier);
            OUT_NOINDENT("#error %s:%d: Value of %s cannot be determined\n",
                         arg->expr->module->source_file_name,
                         arg->expr->_lineno, arg->expr->Identifier);
            break;
    }
...
static int native_long_sign(arg_t *arg, asn1cnst_range_t *r) {
    ...
    if (r->left.type == ARE_VALUE && r->left.value >= 0 &&
        ...
        if (r->el_count == 0 && r->left.value == 0 &&
