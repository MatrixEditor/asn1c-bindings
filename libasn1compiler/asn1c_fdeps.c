...
static asn1c_dep_chain *asn1c_dep_chains_add_new(asn1c_dep_chainset *deps,
                                                 enum asn1c_dep_section section,
                                                 int active) {
        ...
        for (p = strtok(buf, " \t\r\n"); p; p = strtok(NULL, " \t\r\n")) {
            ...
            if (strchr(p, ':')) {
                if (strcmp(p, "COMMON-FILES:") == 0) {
                    ...
                    activate = 0;
                    section = FDEP_RFILL;
                } else if ((arg->flags & A1C_GEN_PYTHON) &&
                           strcmp(p, "PYTHON-FILES:") == 0) {
                    activate = 0;
                    section = FDEP_PYTHON;
                } else {
                    section = FDEP_IGNORE;
...
static asn1c_dep_chain *asn1c_dep_chain_new() {
...
static int asn1c_dep_has_filename(const asn1c_dep_chain *dlist,
                                  const char *filename) {
...
void asn1c_dep_chain_free(asn1c_dep_chain *dc) {
    if (dc) {
...
void asn1c_dep_chainset_free(asn1c_dep_chainset *deps) {
    if (deps) {
        for (size_t i = 0; i < deps->chains_count; i++) {
