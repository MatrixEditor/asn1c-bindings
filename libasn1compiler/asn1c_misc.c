...
static char *res_kwd[] = {
    ...
    "constexpr", "decltype", "delete", "delete", "dynamic_cast", "explicit",
...
void asn1c_prefix_set(const char *prefix) { asn1c_prefix = prefix; }

const char *asn1c_prefix_get() {
    const char *prefix;
 ...
 */
const char *asn1c_make_identifier(enum ami_flags_e flags, asn1p_expr_t *expr,
                                  ...) {
    static char *storage;
    ...
    if (expr) {
         ...
         */
        if (expr->Identifier == NULL) return "Member";
        /*
        ...
        if (expr->_mark & TM_NAMECLASH) {
            ...
            sptr[sptr_cnt++] = expr->module->ModuleName;
                   ...
                   (strcmp(expr->Identifier,
                           expr->reference->ref_expr->Identifier) == 0)) {
        ...
        if (expr->spec_index != -1 && expr->_lineno) {
            ...
            if (expr->_lineno > 0)
                size += 1 + snprintf(buf, sizeof buf, "%dP%d", expr->_lineno,
                                     expr->spec_index);
            else if (expr->_lineno < 0)
                size += 1 + snprintf(buf, sizeof buf, "%d", expr->spec_index);
    ...
    va_start(ap, expr);
    while ((str = va_arg(ap, char *))) size += 1 + strlen(str);
    va_end(ap);
...
const char *asn1c_type_name(arg_t *arg, asn1p_expr_t *expr,
    ...
    if ((top_parent = expr->parent_expr))
        while (top_parent->parent_expr) top_parent = top_parent->parent_expr;
        ...
        case A1TC_REFERENCE:
            ...
            terminal = WITH_MODULE_NAMESPACE(
                ...
                (expr->meta_type == AMT_TYPEREF)
                    ? asn1f_lookup_symbol_ex(arg->asn, expr_ns, expr,
                                             expr->reference)
            ...
            if (_format != TNF_RSAFE && terminal &&
                ((terminal->spec_index != -1) ||
                 (terminal->_mark & TM_NAMECLASH))) {
                exprid = terminal;
                typename = 0;
            }
            ...
            if ((expr->expr_type == ASN_BASIC_REAL &&
                ...
                switch (_format) {
                    case TNF_CONSTYPE:
                        if (expr->expr_type == ASN_BASIC_REAL) {
                            return "double";
                        } else if (asn1c_type_fits_long(arg, expr) ==
                                   FL_FITS_UNSIGN) {
                        ...
                        if (expr->expr_type == ASN_BASIC_REAL) {
                            asn1cnst_range_t *range =
                                asn1constraint_compute_OER_range(
                                    ...
                                    expr->combined_constraints, ACT_EL_RANGE, 0,
                                    0, 0);
                    ...
                    default:
                        ...
                        switch (expr->expr_type) {
                            case ASN_BASIC_INTEGER:
                                typename = "NativeInteger";
                                break;
                            case ASN_BASIC_ENUMERATED:
                                typename = "NativeEnumerated";
                                break;
                            case ASN_BASIC_REAL:
                                typename = "NativeReal";
                                break;
                ...
                if (typename) {
                    if (_format != TNF_INCLUDE) return typename;
                    stdname = 1;
                    break;
                }
            }
    ...
    switch (_format) {
        case TNF_UNMODIFIED:
            return asn1c_make_identifier(
                AMI_MASK_ONLY_SPACES | AMI_NODELIMITER |
                    (stdname ? 0 : AMI_USE_PREFIX),
                0, prefix, MODULE_NAME_OF(exprid),
        ...
        case TNF_RSAFE: /* Recursion-safe type */
            return asn1c_make_identifier(
                AMI_CHECK_RESERVED | AMI_NODELIMITER, 0, "struct", " ", prefix,
                MODULE_NAME_OF(exprid), typename, (char *)0);
    }
...
#define LEFTMIN (-RIGHTMAX - 1) /* of 32-bit integer type */
    ...
    range = asn1constraint_compute_constraint_range(
        expr->Identifier, expr->expr_type, expr->combined_constraints,
        ACT_CT_SIZE, 0, 0, CPR_simulate_fbless_SIZE);
    if (range) {
     ...
     */
        ...
        range->empty_constraint || range->incompatible ||
    ...
    if (left.type == ARE_VALUE && left.value >= 0 && right.type == ARE_VALUE &&
