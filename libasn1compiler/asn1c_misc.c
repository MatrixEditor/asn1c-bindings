...
static int
reserved_py_keyword(const char *str) {
	size_t i;
	for(i = 0; i < sizeof(py_res_kwd) / sizeof(py_res_kwd[0]); i++) {
		if(strcmp(str, py_res_kwd[i]) == 0)
			return 1;
	}
	return 0;
}
...
asn1c_make_identifier(enum ami_flags_e flags, asn1p_expr_t *expr, ...) {
	...
	if(flags & AMI_CHECK_PY_RESERVED)
		size += 1;
	/*
	...
	for(str = 0; str || nextstr; str = nextstr) {
		 ...
		 */
		py_reserved = (flags & AMI_CHECK_PY_RESERVED)
		    && str == first && !nextstr && reserved_py_keyword(str);
