...
static void name_entry_destroy(void *np) {
...
void c_name_clash_finder_destroy() {
    genhash_destroy(used_names_hash);
    used_names_hash = NULL;
}

static void register_global_name(asn1p_expr_t *expr, const char *name) {
    struct intl_name *n;
...
static abuf *construct_base_name(abuf *buf, asn1p_expr_t *expr,
                                 int compound_names, int avoid_keywords,
                                 enum ami_flags_e flag) {
    const char *id;
    ...
    id = asn1c_make_identifier(
        ((avoid_keywords && !buf->length) ? AMI_CHECK_RESERVED : 0) | flag,
        expr, 0);
...
static struct c_names c_name_impl(arg_t *arg, asn1p_expr_t *expr,
                                  int avoid_keywords) {
...
>>>>>>> upstream/vlm_master
    ...
    if (strlen(asn1c_prefix_get()) == 0) {
        ...
        abuf_printf(&b_presence_name, "%s_PR",
                    tmp_compoundable_part_name.buffer);
...
struct c_names c_name(arg_t *arg) { return c_name_impl(arg, arg->expr, 1); }

struct c_names c_expr_name(arg_t *arg, asn1p_expr_t *expr) {
...
const char *c_presence_name(arg_t *arg, asn1p_expr_t *expr) {
    static abuf ab;
...
const char *c_names_format(struct c_names ns) {
    static abuf nbuf;
    abuf_clear(&nbuf);
