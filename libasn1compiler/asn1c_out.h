...
#define REDIR(foo)                 \
    do {                           \
        arg->target->target = foo; \
    } while (0)
#define INDENT_LEVEL arg->target->destination[arg->target->target].indent_level
#define INDENT(val) INDENT_LEVEL += (val)
#define INDENTED(code) \
    do {               \
        INDENT(+1);    \
        do {           \
            code;      \
...
#define GEN_INCLUDE(filename) GEN_POS_INCLUDE(OT_INCLUDES, filename)
#define GEN_POSTINCLUDE(filename) GEN_POS_INCLUDE(OT_POST_INCLUDE, filename)
#define GEN_POS_INCLUDE(pos, filename)           \
    do {                                         \
...
#define GEN_POS_INCLUDE_BASE(pos, expr)                        \
    do {                                                       \
...
#define GEN_DECLARE(type_name, expr)                              \
    do {                                                          \
        int saved_target = arg->target->target;                   \
        REDIR(OT_FUNC_DECLS);                                     \
        OUT_NOINDENT(                                             \
        ...
        if (expr->_type_referenced) {                             \
            OUT_NOINDENT(                                         \
                ...
                "asn_SPC_%s_specs_%d;\n",                         \
                type_name, MKID(expr), expr->_type_unique_index); \
