 ...
 */
int asn1f_process(asn1p_t *asn, enum asn1f_flags flags,
    ...
    if (flags & A1F_EXTENDED_SizeConstraint) {
        ...
        if (arg.debug) {
            arg.debug(-1, "Extended SizeConstraint support enabled");
        }
    }
...
static int asn1f_fix_module__phase_1(arg_t *arg) {
    ...
    TQ_FOR (omod, &arg->asn->modules, mod_next) {
        ...
        if (omod->module_oid && arg->mod->module_oid) {
            ...
            if (asn1p_oid_compare(omod->module_oid, arg->mod->module_oid) ==
                0) {
                FATAL(
                    "ASN.1 module %s in %s "
                ...
                WARNING(
                    "ASN.1 module %s is defined more than once, with different "
                    "OIDs",
                    omod->ModuleName);
                RET2RVAL(1, rvalue);
            }
    ...
    TQ_FOR (expr, &(arg->mod->members), next) {
        ...
        if (expr->ioc_table) {
            for (size_t rn = 0; rn < expr->ioc_table->rows; rn++) {
                for (size_t cn = 0; cn < expr->ioc_table->row[rn]->columns;
                     cn++) {
                    ...
                    ret =
                        asn1f_recurse_expr(arg, asn1f_fix_dereference_defaults);
                    RET2RVAL(ret, rvalue);
...
static int phase_1_1(arg_t *arg, int prm2) {
    ...
    if (expr->lhs_params && expr->spec_index == -1) {
        int i;
        if (!prm2) /* Do not process the parameterized type just yet */
            return 0;
    ...
    DEBUG("=== Now processing \"%s\" (%d/0x%x) at line %d ===",
          expr->Identifier, expr->meta_type, expr->expr_type, expr->_lineno);
...
static int asn1f_fix_simple(arg_t *arg) {
    int rvalue = 0;
    int ret;
...
static int asn1f_fix_constructed(arg_t *arg) {
    int rvalue = 0;
    int ret;
...
static int asn1f_resolve_constraints(arg_t *arg) {
...
static int asn1f_check_constraints(arg_t *arg) {
    ...
    DEBUG("(%s{%d/%d})", arg->expr->Identifier, arg->expr->meta_type,
          arg->expr->expr_type);

    top_parent = asn1f_find_terminal_type(arg, arg->expr);
    if (!top_parent) return 0;
    etype = top_parent->expr_type;
    ...
    for (i = 0; i < sizeof(test_types) / sizeof(test_types[0]); i++) {
        range = asn1constraint_compute_constraint_range(
            arg->expr->Identifier, etype, arg->expr->combined_constraints,
            test_types[i], 0, 0, CPR_noflags /* ignore -fbless-SIZE */);
        if (!range && errno == EPERM) {
            FATAL(
                "This error happened for \"%s\" (meta %d) "
                "at line %d",
                arg->expr->Identifier, arg->expr->meta_type,
                arg->expr->_lineno);
            return -1;
        }
...
static int asn1f_check_duplicate(arg_t *arg) {
    ...
    TQ_FOR (tmparg.mod, &arg->asn->modules, mod_next) {
        ...
        TQ_FOR (tmparg.expr, &(tmparg.mod->members), next) {
            ...
            int oid_exist = (tmparg.expr->module->module_oid &&
                             arg->expr->module->module_oid);
            if ((arg->flags & A1F_COMPOUND_NAMES) &&
                ((!oid_exist && strcmp(tmparg.expr->module->ModuleName,
                                       arg->expr->module->ModuleName)) ||
                 (oid_exist &&
                  asn1p_oid_compare(tmparg.expr->module->module_oid,
                                    arg->expr->module->module_oid)))) {
            ...
            diff_files =
                strcmp(arg->mod->source_file_name, tmparg.mod->source_file_name)
                    ? 1
                    : 0;
            ...
            LOG(critical,
                ...
                arg->expr->Identifier, arg->expr->_lineno, arg->mod->ModuleName,
                tmparg.expr->Identifier, tmparg.expr->_lineno,
                tmparg.mod->ModuleName, diff_files ? " (" : "",
