...
asn1p_expr_t *asn1f_class_access(arg_t *arg, asn1p_expr_t *rhs_pspecs,
    ...
    if (ioclass->expr_type != A1TC_CLASSDEF) {
        if (!(ioclass->_mark & TM_BROKEN)) {
            ...
            FATAL(
                "Class field %s lookup at line %d in something that is not a "
                "class: %s at line %d",
                asn1f_printable_reference(ref), ref->_lineno,
                ioclass->Identifier, ioclass->_lineno);
        }
        errno = EINVAL;
        return NULL;
    }
    ...
    if (classfield == NULL) {
        DEBUG("CLASS %s does not contain field %s", ioclass->Identifier,
    ...
    DEBUG("CLASS %s -> %s (%d)", ioclass->Identifier, classfield->Identifier,
          classfield->expr_type);
