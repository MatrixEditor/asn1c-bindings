...
static int asn1f_check_same_children(arg_t *arg, asn1p_expr_t *a,
                                     asn1p_expr_t *b);
 ...
 */
int asn1f_check_type_compatibility(arg_t *arg, asn1p_expr_t *a,
                                   asn1p_expr_t *b) {
    ...
    DEBUG("(%s:%x@%d, %s:%x@%d)", a->Identifier, atype, a->_lineno,
    ...
    if (a == b) return 0; /* Fairly obviously */
    ...
    if (atype != btype) {
        ...
        if ((atype & ASN_STRING_MASK) && (btype & ASN_STRING_MASK)) {
            /* X.680, B.5 */
            int akm =
                (atype & ASN_STRING_KM_MASK) || atype == ASN_STRING_UTF8String;
            int bkm =
                (btype & ASN_STRING_KM_MASK) || btype == ASN_STRING_UTF8String;
            return (akm == bkm) ? 0 : -1;
        }

        DEBUG("\t%s and %s are not compatible", a->Identifier, b->Identifier);
    ...
    switch (atype) {
        ...
        case ASN_BASIC_ENUMERATED:
            ...
            if (asn1f_check_same_children(arg, a, b)) {
                DEBUG("\tEnumerations are different %s and %s", a->Identifier,
                      b->Identifier);
                return -1;
            }
            return 0;
...
static int asn1f_check_same_children(arg_t *arg, asn1p_expr_t *a,
    ...
    while (1) {
        if (achild->expr_type != bchild->expr_type) return -1;

        if (achild->Identifier && bchild->Identifier) {
            if (strcmp(achild->Identifier, bchild->Identifier)) return -1;
        } else if (!(!achild->Identifier && !bchild->Identifier)) {
            return -1;
        }

        if (achild->value && bchild->value) {
            if (achild->value->type != bchild->value->type) return -1;
    ...
    DEBUG("\t%s:%x@%d and %s:%x@%d are semantically equivalent", a->Identifier,
          a->expr_type, a->_lineno, b->Identifier, b->expr_type, b->_lineno);
