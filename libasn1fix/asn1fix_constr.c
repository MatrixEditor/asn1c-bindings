...
int asn1f_pull_components_of(arg_t *arg) {
    ...
    while ((memb = TQ_REMOVE(&(expr->members), next))) {
        ...
        if (!terminal || (terminal->expr_type != expr->expr_type)) {
            FATAL(
                "COMPONENTS OF at line %d "
                "must reference a %s type",
                coft->_lineno,
                expr->expr_type == ASN_CONSTR_SET ? "SET" : "SEQUENCE");
            TQ_ADD(&list, memb, next);
            r_value = -1;
            continue;
        }
...
int asn1f_fix_constr_ext(arg_t *arg) {
    ...
    while ((v = TQ_REMOVE(&ext_list, next))) TQ_ADD(&(expr->members), v, next);

    if (arg->mod->module_flags & MSF_EXTENSIBILITY_IMPLIED && ext_count == 0) {
...
int asn1f_fix_constr_tag(arg_t *arg, int fix_top_level) {
    asn1p_expr_t *expr = arg->expr;
    asn1p_expr_t *v;
     ...
     */
    if (fix_top_level) {
        if (expr->tag.tag_class == TC_NOCLASS) return r_value;

        if (_asn1f_fix_type_tag(arg, expr)) r_value = -1;
    ...
    TQ_FOR (v, &(expr->members), next) {
        ...
        if (_asn1f_fix_type_tag(arg, v)) r_value = -1;
    }

    if ((arg->mod->module_flags & MSF_AUTOMATIC_TAGS) && !root_tagged) {
        if (ext_tagged) {
            /* X.690: 28.4 */
            FATAL(
                "In %s at line %d: "
...
static int _asn1f_fix_type_tag(arg_t *arg, asn1p_expr_t *expr) {
    ...
    int module_impl_tags =
        (arg->mod->module_flags & (MSF_IMPLICIT_TAGS | MSF_AUTOMATIC_TAGS));
    int r_value = 0;
...
int asn1f_fix_constr_autotag(arg_t *arg) {
    asn1p_expr_t *expr = arg->expr;
    asn1p_expr_t *v;
 ...
 */
int asn1f_check_constr_tags_distinct(arg_t *arg) {
    asn1p_expr_t *expr = arg->expr;
    asn1p_expr_t *v;
    int r_value = 0;
    ...
    TQ_FOR (v, &(expr->members), next) {
        ...
        if (expr->expr_type != ASN_CONSTR_SEQUENCE || v->marker.flags) {
            ...
            for (nv = v; (nv = TQ_NEXT(nv, next));) {
                DEBUG("S/C comparing tags %s s. %s", v->Identifier,
                      nv->Identifier);
                if (_asn1f_compare_tags(arg, v, nv)) r_value = -1;
                if (expr->expr_type == ASN_CONSTR_SEQUENCE && !nv->marker.flags)
                    break;
            }
        }
    }
...
static int _asn1f_compare_tags(arg_t *arg, asn1p_expr_t *a, asn1p_expr_t *b) {
    ...
    ra = asn1f_fetch_outmost_tag(arg->asn, arg->ns, arg->mod, a, &ta,
                                 AFT_IMAGINARY_ANY);
    rb = asn1f_fetch_outmost_tag(arg->asn, arg->ns, arg->mod, b, &tb,
                                 AFT_IMAGINARY_ANY);
    ...
    if (ra == 0 && rb == 0) {
        ...
        if ((ta.tag_value == tb.tag_value && ta.tag_class == tb.tag_class) ||
            ta.tag_value == -1    /* Spread IMAGINARY ANY tag... */
            || tb.tag_value == -1 /* ...it is an evil virus, fear it! */
        ) {
            char tagbuf[2][TAG2STRING_BUFFER_SIZE];
            char *p = (a->expr_type == A1TC_EXTENSIBLE) ? "potentially " : "";
            FATAL(
                "Processing %s at line %d: component \"%s\" at line %d %shas "
                "the same tag "
                "as component \"%s\" at line %d",
                arg->expr->Identifier, arg->expr->_lineno, a->Identifier,
                a->_lineno, p, b->Identifier, b->_lineno);
            FATAL(
                "Consider adding AUTOMATIC TAGS "
                "after module %s DEFINITIONS, "
                "or manually tag components",
                arg->expr->module->ModuleName);
            DEBUG("Tags: %s %s  vs.  %s %s", asn1p_tag2string(&ta, tagbuf[0]),
                  a->Identifier, asn1p_tag2string(&tb, tagbuf[1]),
                  b->Identifier);
            if ((arg->mod->module_flags & MSF_EXTENSIBILITY_IMPLIED) &&
                (a->expr_type == A1TC_EXTENSIBLE) &&
                (b->expr_type == A1TC_EXTENSIBLE)) {
                FATAL(
                    "The previous error is due to "
                    "improper use of "
    ...
    DEBUG("Comparing tags %s:%x <-> %s:%x", a->Identifier, a->expr_type,
