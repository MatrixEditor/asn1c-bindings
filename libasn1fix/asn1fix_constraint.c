...
static int constraint_value_resolve(arg_t *arg, asn1p_value_t **value,
...
int asn1constraint_pullup(arg_t *arg) {
    ...
    if (!ct_parent && !ct_expr) return 0; /* No constraints to consider */
    ...
    if (ct_parent) {
        ...
        if (ct_expr->type == ACT_CA_SET) {
            ...
            for (i = 0; i < ct_expr->el_count; i++) {
                if (asn1p_constraint_insert(expr->combined_constraints,
                                            ct_expr->elements[i])) {
            ...
            asn1p_constraint_insert(expr->combined_constraints, ct_expr);
        }
    } else {
...
int asn1constraint_resolve(arg_t *arg, asn1p_constraint_t *ct,
                           ...
                           enum asn1p_constraint_type_e effective_type) {
    ...
    if (etype != A1TC_INVALID) {
        ret =
            asn1constraint_compatible(etype, real_constraint_type,
    ...
    if (ct->value && ct->value->type == ATV_UNPARSED &&
        etype == A1TC_CLASSDEF) {
...
static void _remove_extensions(arg_t *arg, asn1p_constraint_t *ct,
    ...
    for (i = 0; i < ct->el_count; i++) {
        if (ct->elements[i]->type == ACT_EL_EXT) break;
        if (forgive_last && ct->type == ACT_CA_SET && i + 1 == ct->el_count)
            return;
    ...
    if (i < ct->el_size) ct->elements[i] = 0;
}

static asn1p_ref_t *get_reference_from(asn1p_constraint_t *ct) {
...
static int constraint_type_resolve(arg_t *arg, asn1p_constraint_t *ct) {
    ...
    if (ct_expr->type == ACT_CA_SET) {
        unsigned int i;
        for (i = 0; i < ct_expr->el_count; i++) {
            if (asn1p_constraint_insert(ct, ct_expr->elements[i])) {
                asn1p_constraint_free(ct_expr);
                return -1;
            } else {
                ct_expr->elements[i] = 0;
    ...
    if (rtype && (rtype->meta_type == AMT_VALUESET) && (!rtype->ioc_table))
        return 0;
...
static int constraint_value_resolve(arg_t *arg, asn1p_value_t **value,
    ...
    DEBUG("(\"%s\", within <%s>)", asn1f_printable_value(*value),
