...
static void asn1constraint_range_free_outer(asn1cnst_range_t *cr) {
    if (cr) {
        int i;
        if (cr->elements) {
...
void asn1constraint_range_free(asn1cnst_range_t *cr) {
...
static void _range_remove_element(asn1cnst_range_t *range, int idx) {
    ...
    memmove(&range->elements[idx], &range->elements[idx + 1],
            (range->el_count - idx - 1) * sizeof(range->elements[0]));
    range->el_count--;
...
static int _edge_compare(const asn1cnst_edge_t *el, const asn1cnst_edge_t *er) {
    switch (el->type) {
        case ARE_MIN:
            switch (er->type) {
                case ARE_MIN:
...
static const char *_edge_print(const asn1cnst_edge_t *edge, char *buf,
                               size_t size) {
    *buf = '\0';
    assert(size > 4);
    switch (edge->type) {
...
static const char *_edge_string(const asn1cnst_edge_t *edge) {
    static char buf[128];
    return _edge_print(edge, buf, sizeof(buf));
}

static const char *_range_string(const asn1cnst_range_t *range) {
    static char buf[128];
    char buf0[128];
    char buf1[128];
...
static int _edge_is_within(const asn1cnst_range_t *range,
...
static int _check_edges_within(const asn1cnst_range_t *range,
                               const asn1cnst_range_t *r) {
    if (!_edge_is_within(range, &r->left)) {
        FATAL(
            "Constraint value %s at line %d "
            "is not within "
            "a parent constraint range %s",
            _edge_string(&r->left), r->left.lineno, _range_string(range));
        return -1;
    }
    ...
    if (!_edge_is_within(range, &r->right)) {
        FATAL(
            ...
            "is not within "
            "a parent constraint range %s",
            _edge_string(&r->right), r->left.lineno, _range_string(range));
        return -1;
    }
 ...
 */
static enum asn1cnst_range_narrowing _strongest_narrowing(
    const asn1cnst_range_t *ar, const asn1cnst_range_t *br) {
 ...
 */
static enum asn1cnst_range_narrowing _softest_narrowing(
    const asn1cnst_range_t *ar, const asn1cnst_range_t *br) {
    enum asn1cnst_range_narrowing an = ar->narrowing;
    enum asn1cnst_range_narrowing bn = br->narrowing;
...
enum range_fill_result { RFR_OK, RFR_FAIL, RFR_INCOMPATIBLE };
static enum range_fill_result _range_fill(
    asn1p_value_t *val, const asn1cnst_range_t *minmax, asn1cnst_edge_t *edge,
    asn1cnst_range_t *range, enum asn1p_constraint_type_e type, int lineno) {
    ...
    switch (val->type) {
        ...
        case ATV_QUADRUPLE:
            ...
            return RFR_OK;
        case ATV_STRING:
            if (type != ACT_CT_FROM) return RFR_OK;
            break;
        case ATV_REFERENCED:
            FATAL("Unresolved constraint element \"%s\" at line %d",
                  asn1f_printable_reference(val->value.reference), lineno);
            return RFR_FAIL;
        case ATV_BITVECTOR:
 ...
 */
static int _range_overlap(const asn1cnst_range_t *ra,
                          const asn1cnst_range_t *rb) {
    int lr, rl;
 ...
 */
static asn1cnst_range_t *_range_split(asn1cnst_range_t *ra,
                                      const asn1cnst_range_t *rb) {
    ...
    nr->left = ra->left;
    nr->right = ra->right;
    if (_edge_compare(&ra->left, &rb->left) < 0) nr->left = rb->left;
    if (_edge_compare(&ra->right, &rb->right) > 0) nr->right = rb->right;
...
static int _range_intersection(asn1cnst_range_t *range,
                               const asn1cnst_range_t *with,
                               int strict_edge_check, int is_oer) {
    int ret;
    int i, j;
    ...
    if (strict_edge_check) {
        for (j = -1; j < with->el_count; j++) {
            if (j == -1) {
                ...
                if (_check_edges_within(range, with)) return -1;
            } else {
                if (_check_edges_within(range, with->elements[j])) return -1;
            }
        }
    }
    ...
    for (i = 0; i < range->el_count; i++) {
        for (j = -1; j < with->el_count; j++) {
            ...
            if (_range_overlap(range->elements[i], wel)) break;
        }
        if (j == with->el_count) {
            _range_remove_element(range, i);
            i--;
    ...
    if (range->el_count == 0) range->empty_constraint = 1;

    return 0;
}

static int _range_union(asn1cnst_range_t *range) {
    int i;
    ...
    for (i = 1; i < range->el_count; i++) {
        ...
        if (_range_overlap(ra, rb)) {
            if (_edge_compare(&ra->left, &rb->left) < 0) rb->left = ra->left;
             ...
             */
            if (ra->right.type == ARE_VALUE && rb->left.type == ARE_VALUE &&
...
static int _range_canonicalize(asn1cnst_range_t *range) {
    if (range->el_count == 0) {
        /*
...
asn1cnst_range_t *asn1constraint_compute_OER_range(
    ...
    return asn1constraint_compute_constraint_range(
        dbg_name, expr_type, ct, requested_ct_type, minmax, exmet,
        cpr_flags | CPR_strict_OER_visibility);
}

asn1cnst_range_t *asn1constraint_compute_PER_range(
    const char *dbg_name, asn1p_expr_type_e expr_type,
    const asn1p_constraint_t *ct,
    enum asn1p_constraint_type_e requested_ct_type,
    const asn1cnst_range_t *minmax, int *exmet, enum cpr_flags cpr_flags) {
    ...
    /* Due to peculiarities of PER constraint handling, we don't enable strict
     * PER visibility upfront here. */
    return asn1constraint_compute_constraint_range(
        dbg_name, expr_type, ct, requested_ct_type, minmax, exmet, cpr_flags);
}

asn1cnst_range_t *asn1constraint_compute_JER_range(
    const char *dbg_name, asn1p_expr_type_e expr_type,
    const asn1p_constraint_t *ct,
    enum asn1p_constraint_type_e requested_ct_type,
    const asn1cnst_range_t *minmax, int *exmet, enum cpr_flags cpr_flags) {
...
static asn1cnst_range_t *asn1f_real_range_from_WCOMPS(
    const char *dbg_name, const asn1p_constraint_t *ct) {
...
#define FREE_MEB()             \
    ...
    for (unsigned i = 0; i < ct->el_count; i++) {
        ...
        if (strcmp(asn1p_ref_string(value->value.reference), "mantissa") == 0) {
            dst_range = &mantissa;
        } else if (strcmp(asn1p_ref_string(value->value.reference),
                          "exponent") == 0) {
            dst_range = &exponent;
        ...
        switch (el->el_count) {
            ...
            case 1:
                break;
            default:
                FATAL("Too many constraints (%u) for \"%s\" for REAL",
                      el->el_count, asn1p_ref_string(value->value.reference));
                FREE_MEB();
                return range;
        }
    ...
    if (mantissa->left.type == ARE_VALUE && mantissa->right.type == ARE_VALUE &&
        mantissa->left.value >= -16777215 &&
        mantissa->right.value <= 16777215 && base->left.type == ARE_VALUE &&
        base->right.type == ARE_VALUE && base->left.value == 2 &&
        base->right.value == 2 && exponent->left.type == ARE_VALUE &&
        exponent->right.type == ARE_VALUE && exponent->left.value >= -149 &&
        exponent->right.value <= 104) {
        range->narrowing = NARROW_FLOAT32;
    } else /* X.696 #12.3 */
        if (mantissa->left.type == ARE_VALUE &&
            mantissa->right.type == ARE_VALUE &&
            ...
            exponent->left.type == ARE_VALUE &&
            exponent->right.type == ARE_VALUE &&
...
asn1cnst_range_t *asn1constraint_compute_constraint_range(
    const char *dbg_name, asn1p_expr_type_e expr_type,
    const asn1p_constraint_t *ct,
    enum asn1p_constraint_type_e requested_ct_type,
    const asn1cnst_range_t *minmax, int *exmet, enum cpr_flags cpr_flags) {
    ...
    switch (requested_ct_type) {
        case ACT_EL_RANGE:
            if (exmet == &expectation_met) *exmet = 1;
            break;
        case ACT_CT_FROM:
     ...
     */
    if ((expr_type & ASN_STRING_NKM_MASK)) range->not_PER_visible = 1;
    ...
    switch (ct->type) {
        ...
        case ACT_CT_FROM:
            if (requested_ct_type == ct->type) {
                /*
                 * Specifically requested to process SIZE() or FROM()
                 * constraint.
                 */
                *exmet = 1;
            } else {
        ...
        case ACT_CA_INT: /* SIZE(1..2) ^ FROM("ABCD") */
            ...
            for (i = 0; i < ct->el_count; i++) {
                tmp = asn1constraint_compute_constraint_range(
                    dbg_name, expr_type, ct->elements[i], requested_ct_type,
                    ct->type == ACT_CA_SET ? range : minmax, exmet, cpr_flags);
                if (!tmp) {
                    if (errno == ERANGE) {
                        range->extensible = 1;
                        if (range->extensible) {
                ...
                ret = _range_intersection(
                    range, tmp, ct->type == ACT_CA_SET,
                    cpr_flags & (CPR_strict_OER_visibility |
                                 CPR_strict_JER_visibility));
                _range_free(tmp);
                if (ret) {
                    _range_free(range);
                    errno = EPERM;
    ...
    rfr = _range_fill(vmin, minmax, &range->left, range, requested_ct_type,
                      ct->_lineno);
    if (rfr == RFR_OK) {
        rfr = _range_fill(vmax, minmax, &range->right, range, requested_ct_type,
                          ct->_lineno);
    }
    switch (rfr) {
        case RFR_OK:
...
int main() {
    ...
    ra->right.type = ARE_VALUE;
    ra->right.value = 20;

    /* (10..15) */
    rb->left.type = ARE_VALUE;
    rb->left.value = 10;
    rb->right.type = ARE_VALUE;
    rb->right.value = 15;
