...
static int _asn1f_parse_class_object_data(
    arg_t *, asn1p_expr_t *eclass, struct asn1p_ioc_row_s *row,
    asn1p_wsyntx_t *syntax, const uint8_t *buf, const uint8_t *bend,
    int optional_mode, const uint8_t **newpos, int counter);
static int _asn1f_assign_cell_value(arg_t *arg, struct asn1p_ioc_cell_s *cell,
                                    const uint8_t *buf, const uint8_t *bend,
                                    int counter);
static asn1p_wsyntx_chunk_t *asn1f_next_literal_chunk(
    asn1p_wsyntx_t *syntax, asn1p_wsyntx_chunk_t *chunk, const uint8_t *buf);
...
int asn1f_check_class_object(arg_t *arg) {
    ...
    if (expr->meta_type != AMT_VALUESET || expr->expr_type != A1TC_REFERENCE ||
        !expr->value || expr->value->type != ATV_UNPARSED) {
        return 0;
    }

    eclass = asn1f_find_terminal_type(arg, expr);
    if (!eclass || eclass->meta_type != AMT_OBJECTCLASS ||
    ...
    ret = _asn1f_parse_class_object_data(
        arg, eclass, row, eclass->with_syntax,
        expr->value->value.string.buf + 1,
        expr->value->value.string.buf + expr->value->value.string.size - 1, 0,
        0, 0);
...
static int _asn1f_is_ioc_row_duplicate(asn1p_ioc_table_t *it,
                                       asn1p_ioc_row_t *row) {
    if (!it) {
        return 0;
    }
 ...
 */
static int _asn1f_add_unique_row(arg_t *arg, asn1p_expr_t *expr,
                                 asn1p_ioc_row_t *row) {
 ...
 */
static int _asn1f_parse_object_cb(const uint8_t *buf, size_t size, void *keyp) {
    ...
    if (_asn1f_add_unique_row(arg, eclass, row) != 0) return -1;
    ...
    if (_asn1f_add_unique_row(arg, expr, row) != 0) return -1;

    return 0;
}

static int _asn1f_foreach_unparsed_union(const asn1p_constraint_t *ct_union,
        ...
        if (ct2->type == ACT_EL_VALUE && ct2->value->type == ATV_UNPARSED) {
            if (process &&
                process(ct2->value->value.string.buf + 1,
                        ct2->value->value.string.size - 2, key) != 0) {
                return -1;
            }
            continue;
        }
...
static int _asn1f_foreach_unparsed(arg_t *arg, const asn1p_constraint_t *ct,
                                   int (*process)(const uint8_t *buf,
                                                  size_t size, void *key),
                                   void *keyp) {
    struct parse_object_key *key = keyp;
            ...
            if (ct->value->type == ATV_UNPARSED) {
                if (process &&
                    process(ct->value->value.string.buf + 1,
                            ct->value->value.string.size - 2, key) != 0) {
                    return -1;
                }
            }
            return 0;
...
static int _asn1f_constraint_looks_like_object_set(
...
int asn1f_parse_class_object(arg_t *arg) {
    ...
    DEBUG("Value %s of CLASS %s found at line %d", expr->Identifier,
          eclass->Identifier, expr->_lineno);

    struct parse_object_key key = {.arg = arg,
                                   .expr = expr,
                                   .eclass = eclass,
    ...
    switch (source) {
        case FROM_VALUE:
            if (_asn1f_parse_object_cb(expr->value->value.string.buf + 1,
                                       expr->value->value.string.size - 2,
                                       &key) != 0) {
                return -1;
            }
            break;
...
static int _asn1f_parse_class_object_data_default_syntx(
    arg_t *arg, asn1p_expr_t *eclass, struct asn1p_ioc_row_s *row,
    asn1p_wsyntx_t *syntax, const uint8_t *buf, const uint8_t *bend,
    int optional_mode, const uint8_t **newpos, int counter) {
    int ret;
    ...
    for (; buf < bend; ++buf) {
        ...
        TQ_FOR (cm, &(eclass->members), next) {
            ...
            if (memcmp(buf, cm->Identifier, id_len) == 0) {
                ...
                cell = asn1p_ioc_row_cell_fetch(row, cm->Identifier);
                if (cell == NULL) {
                    if (newpos) *newpos = buf;
                    return -1;
                }
                DEBUG("Reference %s satisfied by %s (%d)", cm->Identifier, buf,
                      p - buf);
...
static int _asn1f_parse_class_object_data_defined_syntx(
    arg_t *arg, asn1p_expr_t *eclass, struct asn1p_ioc_row_s *row,
    asn1p_wsyntx_t *syntax, const uint8_t *buf, const uint8_t *bend,
    int optional_mode, const uint8_t **newpos, int counter) {
        ...
        switch (chunk->type) {
            case WC_LITERAL: {
                ...
                if (bend - buf < token_len ||
                    ...
                    if (!optional_mode) {
                        FATAL(
                            "While parsing object class value %s at line %d: "
                            "Expected: \"%s\", found: \"%s\"",
                            arg->expr->Identifier, arg->expr->_lineno,
                            chunk->content.token, buf);
                    }
                    if (newpos) *newpos = buf;
                    return -1;
                }
            ...
            case WC_FIELD: {
                ...
                if (!next_literal) {
                    ...
                    p = (uint8_t *)strstr(
                        (const char *)buf,
                        (const char *)next_literal->content.token);
                    if (!p) {
                        if (!optional_mode)
                            FATAL("Next literal \"%s\" not found !",
                ...
                cell = asn1p_ioc_row_cell_fetch(row, chunk->content.token);
                if (cell == NULL) {
                    FATAL(
                        "Field reference %s found in WITH SYNTAX {} clause "
                        "does not match actual field in Object Class %s",
                        chunk->content.token, eclass->Identifier,
                        eclass->_lineno);
                    if (newpos) *newpos = buf;
                    return -1;
                }
                DEBUG("Reference %s satisfied by %s (%d)", chunk->content.token,
                      buf, p - buf);
                ret = _asn1f_assign_cell_value(arg, cell, buf, p, counter);
                if (ret) return ret;
                buf = p;
            ...
            case WC_OPTIONALGROUP: {
                ...
                ret = _asn1f_parse_class_object_data(arg, eclass, row,
                                                     chunk->content.syntax, buf,
                                                     bend, 1, &np, counter);
                if (newpos) *newpos = np;
                if (ret && np != buf) return ret;
                buf = np;
            } break;
        }
    }
...
static int _asn1f_parse_class_object_data(
    ...
    if (!eclass->with_syntax) {
        ...
        return _asn1f_parse_class_object_data_default_syntx(
            arg, eclass, row, syntax, buf, bend, optional_mode, newpos,
            counter);
    } else {
        /* WITH SYNTAX {} */
        return _asn1f_parse_class_object_data_defined_syntx(
            arg, eclass, row, syntax, buf, bend, optional_mode, newpos,
            counter);
    }
}
...
static int _asn1f_assign_cell_value(arg_t *arg, struct asn1p_ioc_cell_s *cell,
    ...
    if ((bend - buf) <= 0) {
        FATAL(
            "Assignment warning: empty string is being assigned into %s for %s "
            "at line %d",
            cell->field->Identifier, arg->expr->Identifier, arg->expr->_lineno);
        return -1;
    }
    ...
    for (i = bend - buf - 1; (i > 0) && isspace(mivr[i]); i--) mivr[i] = '\0';
    ...
    if (cell->field->expr_type == A1TC_CLASSFIELD_TFS) {
        ...
        WARNING(
            "asn1c only be able to parse TypeFieldSpec and "
            "FixedTypeValueFieldSpec. Failed when parsing %s at line %d\n",
    ...
    asn = asn1p_parse_buffer(pp, psize, arg->expr->module->source_file_name,
                             arg->expr->_lineno, A1P_NOFLAGS);
    free(pp);
    if (asn == NULL) {
        FATAL(
        ...
        if (expr->value) {
            ...
            expr->Identifier = strdup(
                expr->reference->components[expr->reference->comp_count - 1]
                    .name);
        } else {
    ...
    if (expr->reference && type_expr &&
        ...
        if (asn1f_value_resolve(arg, expr, 0)) {
            ...
            FATAL("Cannot find %s referenced by %s at line %d", mivr,
                  arg->expr->Identifier, arg->expr->_lineno);
            free(mivr);
            return -1;
        }
    ...
    DEBUG("Field %s assignment of %s got %s", cell->field->Identifier, mivr,
          expr->Identifier);
