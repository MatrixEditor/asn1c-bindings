 ...
 */
int asn1f_fix_dereference_values(arg_t *arg) {
 ...
 */
int asn1f_fix_dereference_defaults(arg_t *arg) {
    asn1p_expr_t *expr = arg->expr;
    int r_value = 0;
    ...
    if (expr->marker.default_value) {
        ...
        if (expr->expr_type == A1TC_CLASSFIELD_FTVFS) {
            ...
            child->marker.default_value =
                0; /* ULB: trying to ensure there isn't another pointer to the
                      same value object */
        ...
        if (asn1f_value_resolve(&tmparg, &tmpexpr, 0)) r_value = -1;
