...
int asn1f_fix_enum(arg_t *arg) {
    ...
    if (expr->expr_type != ASN_BASIC_ENUMERATED) return 0; /* Just ignore it */

    DEBUG("(%s)", expr->Identifier);

    used_vals =
        (asn1c_integer_t *)malloc(sizeof(asn1c_integer_t) * used_vals_sz);
    if (!used_vals) {
        FATAL("Out of memory");
        return -1;
    }
    ...
    TQ_FOR (ev, &(expr->members), next) {
        ...
        if (ev->expr_type == A1TC_EXTENSIBLE) {
            ...
            continue;
        } else if (ev->Identifier == NULL || ev->expr_type != A1TC_UNIVERVAL) {
            FATAL(
                "Enumeration %s at line %d: "
        ...
        if (ev->value) {
            switch (ev->value->type) {
                ...
                default:
                    FATAL(
                        "ENUMERATED type %s at line %d "
                        "contain element %s(%s) at line %d",
                        expr->Identifier, expr->_lineno, ev->Identifier,
                        asn1f_printable_value(ev->value), ev->_lineno);
                    rvalue = -1;
                    continue;
            }
        } else {
        ...
        if (ext_marker) {
            if (eval > max_value_ext) {
                ...
                FATAL(
                    ...
                    expr->Identifier, ev->_lineno, ev->Identifier,
                    asn1p_itoa(eval), max_value_buf);
                rvalue = -1;
            }
        }
        ...
        if (unique) {
            ...
            if (used_vals_next >= used_vals_sz) {
                ...
                temp = (asn1c_integer_t *)realloc(
                    used_vals, sizeof(asn1c_integer_t) * new_sz);
                if (!temp) return -1;
                used_vals = temp;
                used_vals_sz = new_sz;
            }
