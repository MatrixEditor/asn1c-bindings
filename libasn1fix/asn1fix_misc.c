...
char const *asn1f_printable_value(asn1p_value_t *v) {
    static char buf[128];
    ...
    switch (v->type) {
        ...
        case ATV_REFERENCED: {
            ...
            for (size_t i = 0; i < ref->comp_count; i++) {
                char *nc;
                if (i) *ptr++ = '.';
                for (nc = ref->components[i].name; *nc; nc++) *ptr++ = *nc;
            }
            *ptr++ = '\0';
        ...
        case ATV_CHOICE_IDENTIFIER: {
            ...
            char const *vptr =
                asn1f_printable_value(v->value.choice_identifier.value);
            char *val;
            ...
            ret = snprintf(managedptr, managedptr_len + 1, "%s: %s", cid, val);
 ...
 * children.
 */
int asn1f_recurse_expr(arg_t *arg, int (*callback)(arg_t *arg)) {
...
int asn1f_check_unique_expr(arg_t *arg) {
    ...
    TQ_FOR (expr, &(arg->expr->members), next) {
        if (expr->Identifier) {
            int ret = asn1f_check_unique_expr_child(arg, expr, 0, "identifier");
            if (ret) rvalue = -1;
        } else {
            /*
...
int asn1f_check_unique_expr_child(arg_t *arg, asn1p_expr_t *child,
    ...
    TQ_FOR (expr, &(arg->expr->members), next) {
        ...
        if (opt_compare) {
            ...
            if (expr->Identifier == NULL || expr->expr_type == A1TC_EXTENSIBLE)
                continue;
        ...
        if (ret == 0) {
            FATAL(
                ...
                "\"%s\" at line %d",
                expr->Identifier, expr->_lineno, opt_property_name,
                child->Identifier, child->_lineno);
            return -1;
        }
    }
...
int asn1f_count_children(asn1p_expr_t *expr) {
    asn1p_expr_t *child;
    int count = 0;
...
int asn1f_make_known_external_type(const char *type_name) {
    char *tname;
...
int asn1f_check_known_external_type(const char *type_name) {
