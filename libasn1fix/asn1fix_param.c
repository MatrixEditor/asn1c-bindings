...
static int compare_specializations(const asn1p_expr_t *a,
                                   const asn1p_expr_t *b);
static asn1p_expr_t *find_target_specialization_byvalueset(
    resolver_arg_t *rarg, asn1p_constraint_t *ct);
static asn1p_expr_t *find_target_specialization_byref(resolver_arg_t *rarg,
                                                      asn1p_ref_t *ref);
static asn1p_expr_t *find_target_specialization_bystr(resolver_arg_t *rarg,
                                                      char *str);

asn1p_expr_t *asn1f_parameterization_fork(arg_t *arg, asn1p_expr_t *expr,
                                          asn1p_expr_t *rhs_pspecs) {
     ...
     */
    for (npspecs = 0; npspecs < expr->specializations.pspecs_count; npspecs++) {
        if (compare_specializations(
                rhs_pspecs, expr->specializations.pspec[npspecs].rhs_pspecs) ==
            0) {
    ...
    DEBUG("Forking parameterization at %d for %s (%d alr)", rhs_pspecs->_lineno,
          expr->Identifier, expr->specializations.pspecs_count);
    ...
    exc->rhs_pspecs = asn1p_expr_clone_with_resolver(
        expr->rhs_pspecs ? expr->rhs_pspecs : rhs_pspecs, resolve_expr, &rarg);

    /* Passing arguments to members and type references */

    target = TQ_FIRST(&expr->members);
    TQ_FOR (m, &exc->members, next) {
        m->rhs_pspecs = asn1p_expr_clone_with_resolver(
            target->rhs_pspecs ? target->rhs_pspecs : exc->rhs_pspecs,
            resolve_expr, &rarg);
...
static asn1p_expr_t *resolve_expr(asn1p_expr_t *expr_to_resolve,
                                  void *resolver_arg) {
    ...
    DEBUG("Resolving %s (meta %d)", expr_to_resolve->Identifier,
          expr_to_resolve->meta_type);

    if (expr_to_resolve->meta_type == AMT_TYPEREF) {
        expr =
            find_target_specialization_byref(rarg, expr_to_resolve->reference);
        if (!expr) return NULL;
    } else if (expr_to_resolve->meta_type == AMT_VALUE) {
        if (!expr_to_resolve->value) return NULL;
        expr =
            find_target_specialization_bystr(rarg, expr_to_resolve->Identifier);
        if (!expr) return NULL;
    } else if (expr_to_resolve->meta_type == AMT_VALUESET) {
        assert(expr_to_resolve->constraints);
        expr = find_target_specialization_byvalueset(
            rarg, expr_to_resolve->constraints);
        if (!expr) return NULL;
    } else {
        errno = ESRCH;
        return NULL;
    ...
    DEBUG("Found target %s (%d/%x)", expr->Identifier, expr->meta_type,
          expr->expr_type);
    if (expr->meta_type == AMT_TYPE || expr->meta_type == AMT_VALUE ||
        expr->meta_type == AMT_TYPEREF || expr->meta_type == AMT_VALUESET) {
        DEBUG("Target is a simple type %s", ASN_EXPR_TYPE2STR(expr->expr_type));
        nex = asn1p_expr_clone(expr, 0);
        free(nex->Identifier);
        nex->Identifier = expr_to_resolve->Identifier
                              ? strdup(expr_to_resolve->Identifier)
                              : 0;
        if (expr->meta_type == AMT_TYPEREF) {
            asn1p_ref_t *ref = expr->reference;
            rarg->resolved_name = calloc(
                1, strlen(rarg->original_expr->Identifier) +
                       strlen(ref->components[ref->comp_count - 1].name) + 2);
            sprintf(rarg->resolved_name, "%s_%s",
                    rarg->original_expr->Identifier,
                    ref->components[ref->comp_count - 1].name);
        } else if (expr->meta_type == AMT_VALUESET) {
            asn1p_constraint_t *ct = expr->constraints;
            if (ct->type == ACT_EL_TYPE) {
                asn1p_ref_t *ref =
                    ct->containedSubtype->value.v_type->reference;
                rarg->resolved_name =
                    strdup(ref->components[ref->comp_count - 1].name);
            }
        }
        return nex;
    } else {
        FATAL(
            "Feature not implemented for %s (%d/%x), "
            "please contact the asn1c author",
            rarg->original_expr->Identifier, expr->meta_type, expr->expr_type);
        errno = EPERM;
        return NULL;
    }
...
static asn1p_expr_t *find_target_specialization_bystr(resolver_arg_t *rarg,
    ...
    for (i = 0; i < rarg->lhs_params->params_count;
        ...
        if (strcmp(param->argument, refstr)) continue;

        return target;
    }
    if (i != rarg->lhs_params->params_count) {
        FATAL(
            "Parameterization of %s failed: "
