...
static asn1p_expr_t *asn1f_find_terminal_thing(arg_t *arg, asn1p_expr_t *expr,
                                               enum ftt_what);
static int asn1f_compatible_with_exports(arg_t *arg, asn1p_module_t *mod,
                                         const char *name);
...
asn1p_expr_t *asn1f_lookup_child(asn1p_expr_t *tc, const char *name) {
    ...
    TQ_FOR (child_tc, &(tc->members), next) {
        if (child_tc->Identifier && strcmp(child_tc->Identifier, name) == 0) {
            return child_tc;
        }
    }
...
static asn1p_expr_t *asn1f_lookup_in_module(asn1p_module_t *mod,
                                            const char *name) {
...
asn1p_module_t *asn1f_lookup_module(arg_t *arg, const char *module_name,
                                    const asn1p_oid_t *oid, int oid_option) {
    ...
    if (oid == 0) {
        ...
        TQ_FOR (xp, &(arg->mod->imports), xp_next) {
            if (strcmp(module_name, xp->fromModuleName)) continue;
            if (oid) {
                FATAL(
                    "Ambiguous reference: "
                    "%s "
    ...
    TQ_FOR (mod, &(arg->asn->modules), mod_next) {
        if (oid) {
            if (mod->module_oid) {
                ...
                if (oid_option == XPT_WITH_SUCCESSORS) {
                    if (r == oid->arcs_count &&
                        r == mod->module_oid
                                 ->arcs_count) /* positive and last arc */
                        r = 0;
                } else if (oid_option == XPT_WITH_DESCENDANTS) {
                    if (oid->arcs_count == (-1 - r)) r = 0;
                }
                if (0 == r) {
        ...
        if (strcmp(module_name, mod->ModuleName) == 0) return mod;
    }
    if (ret == NULL) {
...
static asn1p_expr_t *asn1f_lookup_symbol_impl(arg_t *arg,
    ...
    DEBUG("Lookup (%s) in %s for line %d", asn1f_printable_reference(ref),
          asn1_namespace_string(initial_namespace), ref->_lineno);
    ...
    } else if (ref->comp_count == 2 && ref->components[1].name[0] != '&') {
        modulename = ref->components[0].name;
        identifier = ref->components[1].name;
    } else if (ref->comp_count > 1 && isupper(ref->components[0].name[0]) &&
    ...
    for (ssize_t ns_item = my_namespace->elements_count - 1; ns_item >= 0;
        ...
        switch (ns_el->selector) {
            ...
            case NAM_SPACE: {
                 ...
                 */
                ref_tc =
                    asn1f_lookup_in_module(ns_el->u.space.module, identifier);
                if (ref_tc) {
                    ...
                    if (rhs_pspecs && ref_tc->lhs_params) {
                        ...
                        ref_tc = asn1f_parameterization_fork(arg, ref_tc,
                                                             rhs_pspecs);
                    }
                ...
                imports_from = asn1f_lookup_in_imports(
                    arg, ns_el->u.space.module, identifier);
                if (imports_from) {
                    if (namespace_switches++ > 10 /* Arbitrary constant */) {
                        FATAL(
                            "Excessive circular referencing detected in "
                            "namespace "
                            "%s for %s at "
                            "line %d",
                    ...
                    DISPOSE_OF_MY_NAMESPACE();
                    my_namespace =
                        asn1_namespace_new_from_module(imports_from, 1);
...
asn1p_expr_t *asn1f_find_terminal_type(arg_t *arg, asn1p_expr_t *expr) {
    return asn1f_find_terminal_thing(arg, expr, FTT_TYPE);
}

asn1p_expr_t *asn1f_find_terminal_value(arg_t *arg, asn1p_expr_t *expr) {
...
static asn1p_expr_t *asn1f_find_terminal_thing(arg_t *arg, asn1p_expr_t *expr,
    ...
    switch (what) {
        ...
        case FTT_VALUE:
            ...
            DEBUG("%s(%s->%s) meta %d for line %d", "VALUE", expr->Identifier,
                  asn1f_printable_reference(ref), expr->meta_type,
                  expr->_lineno);
    ...
    DEBUG("%s(%s->%s) for line %d", (what == FTT_VALUE) ? "VALUE" : "TYPE",
          expr->Identifier, asn1f_printable_reference(ref), expr->_lineno);
    ...
    if (tc == NULL) {
        ...
        if (tc == NULL) {
            DEBUG("\tSymbol \"%s\" not found: %s",
                  asn1f_printable_reference(ref), strerror(errno));
            return NULL;
        }
    }
...
static int asn1f_compatible_with_exports(arg_t *arg, asn1p_module_t *mod,
    ...
    TQ_FOR (item, &(exports->xp_members), next) {
        if (strcmp(item->Identifier, name) == 0) return 0;
    }
    ...
    if (!(arg->expr->_mark & TM_BROKEN)) {
        ...
        FATAL(
            ...
            mod->ModuleName, mod->source_file_name, name, arg->expr->_lineno);
    }
