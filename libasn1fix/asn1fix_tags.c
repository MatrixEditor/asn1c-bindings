...
#define ADD_TAG(skip, newtag)                                                  \
    do {                                                                       \
        ...
        __p = realloc((*tags), sizeof(struct asn1p_type_tag_s) * (count + 1)); \
        if (!__p) return -1;                                                   \
        *tags = __p;                                                           \
        (*tags)[count++] = newtag;                                             \
...
/* X.691, #22.2 */
static int asn1f_fetch_minimal_choice_root_tag(arg_t *arg,
                                               struct asn1p_type_tag_s *tag,
                                               enum asn1f_aft_flags_e flags);

static int asn1f_fetch_tags_impl(arg_t *arg, struct asn1p_type_tag_s **tags,
                                 int count, int skip,
                                 enum asn1f_aft_flags_e flags) {
    ...
    if (expr->tag.tag_class != TC_NOCLASS) ADD_TAG(skip, expr->tag);
    ...
    if (expr->meta_type == AMT_TYPE) {
        ...
        if (tt.tag_value == 0) {
            if (expr->expr_type == ASN_TYPE_ANY && (flags & AFT_IMAGINARY_ANY))
                tt.tag_value = -1;
                ...
                return -1;
            else if (asn1f_fetch_minimal_choice_root_tag(arg, &tt, flags))
                return -1;
        }
        ADD_TAG(skip, tt);
        return count;
    ...
    if (expr->meta_type == AMT_TYPEREF) {
        ...
        if (nexpr == NULL) {
            ...
            FATAL(
                ...
                expr->Identifier, asn1f_printable_reference(expr->reference));
            return -1;
        } else {
            arg->expr = nexpr;
        }
        if (expr->_mark & TM_RECURSION) return -1;
...
int asn1f_fetch_outmost_tag(asn1p_t *asn, asn1_namespace_t *ns,
...
int asn1f_fetch_tags(asn1p_t *asn, asn1_namespace_t *ns, asn1p_module_t *mod,
                     asn1p_expr_t *expr, struct asn1p_type_tag_s **tags_r,
                     enum asn1f_aft_flags_e flags) {
    arg_t arg;
    struct asn1p_type_tag_s *tags = 0;
    int count;
