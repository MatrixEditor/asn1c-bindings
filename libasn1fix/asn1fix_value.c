...
int asn1f_value_resolve(arg_t *arg, asn1p_expr_t *expr,
    ...
    if (expr->value->type != ATV_REFERENCED) return 0;

    DEBUG("(=\"%s\", %x%s%s)", asn1f_printable_value(expr->value),
          expr->expr_type, opt_constr_type ? ", " : "",
          opt_constr_type ? asn1p_constraint_type2str(*opt_constr_type) : "");
    ...
    if (type_expr == 0) {
        if (errno == EEXIST) {
            DEBUG("External type for %s at line %d", expr->Identifier,
                  expr->_lineno);
            return 0;
        } else {
            FATAL("Terminal type for %s at line %d not found", expr->Identifier,
                  expr->_lineno);
            return -1;
        }
    }
    ...
    if (asn1f_look_value_in_type(arg, type_expr, expr) == -1) {
        FATAL("Value not found in type for %s at line %d", expr->Identifier,
              expr->_lineno);
        return -1;
    }
    ...
    if (value_expr) {
        DEBUG("Terminal value for %s->%s is %s at line %d", expr->Identifier,
              asn1f_printable_value(expr->value), value_expr->Identifier,
              value_expr->_lineno);
    } else {
        FATAL("Terminal value for %s->%s not found", expr->Identifier,
    ...
    if (val_type_expr) {
        ...
        FATAL("Terminal type of value %s->%s not found", expr->Identifier,
              asn1f_printable_value(expr->value));
        return -1;
    }
    ...
    if (opt_constr_type)
        ret = asn1constraint_compatible(val_type_expr->expr_type,
                                        *opt_constr_type,
                                        0 /* must not matter here */);
    else
        ret = asn1f_check_type_compatibility(arg, type_expr, val_type_expr);
    if (ret == -1) {
        switch (type_expr->expr_type) {
            default:
                if (!(type_expr->expr_type & ASN_STRING_MASK)) break;
                /* Compatibility rules are not defined */
                /* Fall through */
            case ASN_BASIC_INTEGER:
            case ASN_BASIC_ENUMERATED:
                FATAL(
                    "Incompatible type of \"%s\" (%s) at line %d "
                    "with \"%s\" (%s) at line %d",
                    type_expr->Identifier,
                    ASN_EXPR_TYPE2STR(type_expr->expr_type), type_expr->_lineno,
        ...
        WARNING(
            "Possibly incompatible type of \"%s\" (%s) at line %d "
            "with \"%s\" (%s) at line %d",
            type_expr->Identifier, ASN_EXPR_TYPE2STR(type_expr->expr_type),
            type_expr->_lineno, val_type_expr->Identifier,
            ASN_EXPR_TYPE2STR(val_type_expr->expr_type),
            val_type_expr->_lineno);
        return 1;
    }

    if (asn1f_look_value_in_type(arg, val_type_expr, expr) == -1) return -1;
    ...
    if (ret == -1) {
        FATAL("Value %s cannot be copied from line %d to line %d",
              asn1f_printable_value(value_expr->value), value_expr->_lineno,
              expr->_lineno);
        return -1;
    }

    DEBUG("Final value for \"%s\" at line %d is %s", expr->Identifier,
          expr->_lineno, asn1f_printable_value(expr->value));
...
static int _asn1f_copy_value(arg_t *arg, asn1p_expr_t *to, asn1p_expr_t *from) {
    ...
    if (v) {
        ...
        to->value = v;
        DEBUG(
            "Copied value %s from \"%s\" on line %d "
            "to \"%s\" on line %d",
            asn1f_printable_value(v), from->Identifier, from->_lineno,
            to->Identifier, to->_lineno);
        return 0;
    } else {
        return -1;
    }
...
int asn1f_look_value_in_type(arg_t *arg, asn1p_expr_t *type_expr,
    ...
    DEBUG("(for %s in %s %x) for line %d",
          asn1f_printable_value(value_expr->value), type_expr->Identifier,
          type_expr->expr_type, value_expr->_lineno);
    ...
    DEBUG(
        "Looking into a type %s at line %d for %s at line %d: %s",
        type_expr->Identifier, type_expr->_lineno, identifier,
        value_expr->_lineno,
        child_expr ? asn1f_printable_value(child_expr->value) : "<not found>");
        ...
        if (!child_expr->value) {
            ...
            switch (type_expr->expr_type) {
                ...
                case ASN_BASIC_ENUMERATED:
                    ...
                    break;
                default:
                    WARNING("Unexpected type %s for %s", type_expr->expr_type,
                            type_expr->Identifier);
                    return -1;
            }
            arg->expr = saved_expr;
        }
        if (child_expr->value && _asn1f_copy_value(arg, value_expr, child_expr))
            return -1;
        /* Fall through */
    }
