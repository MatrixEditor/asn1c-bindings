...
static int check(const char *fname, enum asn1p_flags parser_flags,
...
static int check(const char *fname, enum asn1p_flags parser_flags,
     ...
     */
    if (!expected_parseable) fprintf(stderr, "Expecting error...\n");
    ...
    if (asn == NULL) {
        if (expected_parseable) {
            ...
            fprintf(stderr, "Previous error is EXPECTED, no worry\n");
        }
    ...
    if (r_value == 0) {
        ...
        if (expected_fix_code) fprintf(stderr, "Expecting some problems...\n");
        ...
        if (ret) {
            if (ret == expected_fix_code) {
                ...
                fprintf(stderr, "Cannot process file \"%s\": %d\n", fname, ret);
                r_value = -1;
            }
     ...
     */
    if (r_value == 0) {
        if (post_fix_check(asn)) r_value = -1;
    }
    ...
    TQ_FOR (mod, &(asn->modules), mod_next) {
        TQ_FOR (expr, &(mod->members), next) {
            ...
            if (strncmp(expr->Identifier, "check-", 6) == 0) {
                if (post_fix_check_element(mod, expr)) r_value = -1;
            }
        }
    }
...
static int post_fix_check_element(asn1p_module_t *mod,
                                  asn1p_expr_t *check_expr) {
    ...
    if (check_expr->expr_type != ASN_BASIC_INTEGER ||
        ...
        fprintf(stderr,
                ...
                "%d at line %d of %s\n",
                check_expr->Identifier, check_expr->expr_type,
                check_expr->_lineno, mod->source_file_name);
        return -1;
    }
    ...
    if (value == NULL || value->type != ATV_INTEGER) {
        fprintf(stderr,
                ...
                "%d at line %d of %s\n",
                check_expr->Identifier, value ? (signed)value->type : -1,
                expr->_lineno, mod->source_file_name);
        return -1;
    }
    ...
    if (expr == NULL) {
        fprintf(stderr,
                ...
                name, name, check_expr->_lineno, mod->source_file_name);
        return -1;
    }
    ...
    if (0 && expr->expr_type != check_expr->expr_type) {
        fprintf(stderr,
                ...
                check_expr->expr_type, name, mod->source_file_name);
        return -1;
    }

    if (expr->value == NULL || expr->value->type != value->type) {
        fprintf(stderr,
                "CHECKER: Value of \"%s\" (\"%s\", type=%d) at line %d "
                "does not have desired type %d as requested by "
                "\"check-%s\" in %s\n",
                expr->Identifier, asn1f_printable_value(expr->value),
                expr->value->type, expr->_lineno, value->type, name,
