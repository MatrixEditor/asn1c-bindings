...
asn1p_ioc_table_t *asn1p_ioc_table_new() {
    asn1p_ioc_table_t *it = calloc(1, sizeof(*it));
    assert(it);
    return it;
}

void asn1p_ioc_table_add(asn1p_ioc_table_t *it, asn1p_ioc_row_t *row) {
    assert(it);
...
void asn1p_ioc_table_free(asn1p_ioc_table_t *it) {
    if (it) {
...
size_t asn1p_ioc_table_max_identifier_length(asn1p_ioc_table_t *it) {
    size_t max_length = 0;
    if (it) {
        for (size_t i = 0; i < it->rows; i++) {
...
size_t asn1p_ioc_row_max_identifier_length(asn1p_ioc_row_t *row) {
    size_t max_length = 0;
    if (row) {
...
asn1p_ioc_row_t *asn1p_ioc_row_new(asn1p_expr_t *oclass) {
    asn1p_ioc_row_t *row;
    asn1p_expr_t *field;
    int columns = 0;
    ...
    TQ_FOR (field, &oclass->members, next) columns++;
...
void asn1p_ioc_row_delete(asn1p_ioc_row_t *row) {
    if (row) {
        if (row->column) {
            for (size_t i = 0; i < row->columns; i++) {
...
int asn1p_ioc_row_match(const asn1p_ioc_row_t *a, const asn1p_ioc_row_t *b) {
    assert(a && b);

    if (a->columns != b->columns) return -1; /* Bad! */

    for (size_t i = 0; i < a->columns; i++) {
        assert(a->column[i].field);
        assert(b->column[i].field);
        if (strcmp(a->column[i].field->Identifier,
                   b->column[i].field->Identifier) != 0) {
            return -1; /* Bad! */
        }
...
struct asn1p_ioc_cell_s *asn1p_ioc_row_cell_fetch(asn1p_ioc_row_t *row,
...
asn1p_wsyntx_chunk_t *asn1p_wsyntx_chunk_new() {
...
void asn1p_wsyntx_chunk_free(asn1p_wsyntx_chunk_t *wc) {
    if (wc) {
        switch (wc->type) {
            case WC_LITERAL:
            case WC_WHITESPACE:
            case WC_FIELD:
                free(wc->content.token);
                break;
...
asn1p_wsyntx_chunk_t *asn1p_wsyntx_chunk_clone(asn1p_wsyntx_chunk_t *wc) {
...
asn1p_wsyntx_t *asn1p_wsyntx_new() {
    asn1p_wsyntx_t *wx;
...
void asn1p_wsyntx_free(asn1p_wsyntx_t *wx) {
    if (wx) {
        asn1p_wsyntx_chunk_t *wc;
...
asn1p_wsyntx_t *asn1p_wsyntx_clone(asn1p_wsyntx_t *wx) {
    asn1p_wsyntx_t *nw;
...
asn1p_wsyntx_chunk_t *asn1p_wsyntx_chunk_fromstring(char *token, int do_copy) {
    asn1p_wsyntx_chunk_t *wc;
