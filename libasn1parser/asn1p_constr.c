...
void asn1p_constraint_set_source(asn1p_constraint_t *ct,
...
void asn1p_constraint_free(asn1p_constraint_t *ct) {
    if (ct) {
        ...
        if (ct->elements) {
            while (ct->el_count--) {
                asn1p_constraint_free(ct->elements[ct->el_count]);
            }
            free(ct->elements);
        }
...
asn1p_constraint_t *asn1p_constraint_clone(asn1p_constraint_t *src) {
    return asn1p_constraint_clone_with_resolver(src, 0, 0);
}

asn1p_constraint_t *asn1p_constraint_clone_with_resolver(
    asn1p_constraint_t *src, asn1p_value_t *(*vr)(asn1p_value_t *, void *varg),
    void *varg) {
    asn1p_constraint_t *clone;

#define CLONE(field, func)                             \
    do {                                               \
        if (src->field) {                              \
    ...
    if (clone) {
        ...
        for (i = 0; i < src->el_count; i++) {
            ...
            t = asn1p_constraint_clone_with_resolver(src->elements[i], vr,
                                                     varg);
            if (!t) {
                asn1p_constraint_free(clone);
                return NULL;
 ...
 */
static int asn1p_constraint_make_memory(asn1p_constraint_t *ct) {
...
int asn1p_constraint_insert(asn1p_constraint_t *into,
                            asn1p_constraint_t *what) {
    assert(into);
    assert(what);

    if (asn1p_constraint_make_memory(into)) return -1;
...
int asn1p_constraint_prepend(asn1p_constraint_t *before,
                             asn1p_constraint_t *what) {
    assert(before);
    assert(what);

    if (asn1p_constraint_make_memory(before)) return -1;
...
const char *asn1p_constraint_type2str(enum asn1p_constraint_type_e type) {
    switch (type) {
        case ACT_INVALID:
            return "INVALID";
        case ACT_EL_TYPE:
...
const asn1p_constraint_t *asn1p_get_component_relation_constraint(
    asn1p_constraint_t *ct) {
    if (ct) {
        if (ct->type == ACT_CA_CRC) return ct;
