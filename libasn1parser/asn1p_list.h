...
#define TQ_FOR(var, head, field) \
    for ((var) = TQ_FIRST((head)); (var); (var) = TQ_NEXT((var), field))

/* MSVC does not have typeof(), cannot prevent side effects! */
#define TQ_ADD(head, xel, field)                   \
    do {                                           \
        typeof(xel) __el = (xel);                  \
...
#define TQ_CONCAT(head1, head2, field)            \
    do {                                          \
        if (TQ_FIRST(head2)) {                    \
...
#define TQ_REMOVE(head, field)                                               \
    ({                                                                       \
        ...
        __fel = TQ_FIRST((head));                                            \
        if (__fel == 0 || (TQ_FIRST((head)) = TQ_NEXT(__fel, field)) == 0) { \
            (head)->tq_tail = &TQ_FIRST((head));                             \
        } else {                                                             \
