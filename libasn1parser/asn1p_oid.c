...
asn1p_oid_t *asn1p_oid_construct(asn1p_oid_arc_t *arc, int narcs) {
    asn1p_oid_t *oid;

    if (narcs <= 0) return NULL;
...
asn1p_oid_t *asn1p_oid_new() { return calloc(1, sizeof(asn1p_oid_t)); }

int asn1p_oid_add_arc(asn1p_oid_t *oid, asn1p_oid_arc_t *template) {
    void *p;
...
void asn1p_oid_free(asn1p_oid_t *oid) {
    if (oid) {
        if (oid->arcs) {
...
asn1p_oid_arc_t *asn1p_oid_arc_new(const char *optName,
    ...
    if (arc) {
        if (optName) arc->name = strdup(optName);
...
void asn1p_oid_arc_free(asn1p_oid_arc_t *arc) {
    if (arc) {
        free(arc->name);
        free(arc);
    }
...
int asn1p_oid_compare(const asn1p_oid_t *a, const asn1p_oid_t *b) {
    ...
    for (i = 0;; i++) {
        ...
        if (b->arcs_count > i) {
            if (a->arcs_count <= i) return -1 - i;
        } else if (a->arcs_count > i) {
            if (b->arcs_count <= i) return 1 + i;
