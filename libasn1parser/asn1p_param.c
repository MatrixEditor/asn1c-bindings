...
void asn1p_paramlist_free(asn1p_paramlist_t *pl) {
    if (pl) {
        if (pl->params) {
...
int asn1p_paramlist_add_param(asn1p_paramlist_t *pl, asn1p_ref_t *gov,
                              char *arg) {
    if (!pl || !arg) {
        errno = EINVAL;
        return -1;
    ...
    if (pl->params_count == pl->params_size) {
        ...
        void *p;
        p = realloc(pl->params, newsize * sizeof(pl->params[0]));
        if (p) {
            pl->params = p;
            pl->params_size = newsize;
            memset(&pl->params[pl->params_count], 0,
                   (newsize - pl->params_size) * sizeof(pl->params[0]));
        } else {
            return -1;
        }
    }
    ...
    if (gov) {
        ...
        if (pl->params[pl->params_count].governor == NULL) return -1;
    } else {
...
asn1p_paramlist_t *asn1p_paramlist_clone(asn1p_paramlist_t *pl) {
    ...
    if (newpl) {
        ...
        for (i = 0; i < pl->params_count; i++) {
            if (asn1p_paramlist_add_param(newpl, pl->params[i].governor,
