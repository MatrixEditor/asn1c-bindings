...
void asn1p_value_set_source(asn1p_value_t *value, asn1p_module_t *module,
                            int lineno) {
    if (value) {
        switch (value->type) {
            case ATV_TYPE:
...
int asn1p_value_compare(const asn1p_value_t *a, const asn1p_value_t *b) {
    if (a->type != b->type) {
        return -1;
    }
    ...
    switch (a->type) {
        ...
        case ATV_BITVECTOR:
            if (a->value.binary_vector.size_in_bits !=
                ...
                memcmp(a->value.binary_vector.bits, b->value.binary_vector.bits,
                       (a->value.binary_vector.size_in_bits + 7) >> 3) != 0) {
                return -1;
            }
            return 0;
        case ATV_VALUESET:
        ...
        case ATV_CHOICE_IDENTIFIER:
            if (strcmp(a->value.choice_identifier.identifier,
                       b->value.choice_identifier.identifier) != 0) {
                return -1;
            }
...
asn1p_value_t *asn1p_value_fromconstr(asn1p_constraint_t *ct, int do_copy) {
    if (ct) {
        asn1p_value_t *v = calloc(1, sizeof *v);
        if (v) {
            if (do_copy) {
                v->value.constraint = asn1p_constraint_clone(ct);
...
asn1p_value_t *asn1p_value_frombits(uint8_t *bits, int size_in_bits,
                                    int do_copy) {
    if (bits) {
        asn1p_value_t *v = calloc(1, sizeof *v);
...
asn1p_value_t *asn1p_value_fromdouble(double d) {
    asn1p_value_t *v = calloc(1, sizeof *v);
    if (v) {
        v->value.v_double = d;
        v->type = ATV_REAL;
...
asn1p_value_t *asn1p_value_fromint(asn1c_integer_t i) {
    asn1p_value_t *v = calloc(1, sizeof *v);
    if (v) {
        v->value.v_integer = i;
        v->type = ATV_INTEGER;
...
asn1p_value_t *asn1p_value_fromtype(asn1p_expr_t *expr) {
    asn1p_value_t *v = calloc(1, sizeof *v);
    if (v) {
        v->value.v_type = expr;
        v->type = ATV_TYPE;
...
asn1p_value_t *asn1p_value_clone(asn1p_value_t *v) {
    return asn1p_value_clone_with_resolver(v, 0, 0);
}

asn1p_value_t *asn1p_value_clone_with_resolver(
    asn1p_value_t *v, asn1p_value_t *(*resolver)(asn1p_value_t *, void *rarg),
    void *rarg) {
        ...
        switch (v->type) {
            ...
            case ATV_BITVECTOR:
                return asn1p_value_frombits(v->value.binary_vector.bits,
                                            v->value.binary_vector.size_in_bits,
                                            1);
            case ATV_REFERENCED:
                if (resolver) {
...
void asn1p_value_free(asn1p_value_t *v) {
    if (v) {
        switch (v->type) {
            case ATV_NOVALUE:
            case ATV_NULL:
