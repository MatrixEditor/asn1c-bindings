...
static int asn1print_module(asn1p_t *asn, asn1p_module_t *mod,
                            enum asn1print_flags flags);
static int asn1print_oid(int prior_len, asn1p_oid_t *oid,
                         enum asn1print_flags flags);
...
static int asn1print_params(const asn1p_paramlist_t *pl,
                            enum asn1print_flags flags);
static int asn1print_with_syntax(const asn1p_wsyntx_t *wx,
                                 enum asn1print_flags flags);
static int asn1print_constraint(const asn1p_constraint_t *,
                                enum asn1print_flags);
static int asn1print_value(const asn1p_value_t *val,
                           enum asn1print_flags flags);
static int asn1print_expr(asn1p_t *asn, asn1p_module_t *mod, asn1p_expr_t *tc,
                          enum asn1print_flags flags, int level);
static int asn1print_expr_dtd(asn1p_t *asn, asn1p_module_t *mod,
...
static int asn1print_module(asn1p_t *asn, asn1p_module_t *mod,
    ...
    if (flags & APF_PRINT_XML_DTD) safe_printf("<!-- ASN.1 module\n");
    ...
    if (flags & APF_PRINT_XML_DTD) {
        if (mod->source_file_name && strcmp(mod->source_file_name, "-"))
    ...
    if (mod->module_flags & MSF_EXPLICIT_TAGS) safe_printf(" EXPLICIT TAGS");
    if (mod->module_flags & MSF_IMPLICIT_TAGS) safe_printf(" IMPLICIT TAGS");
    if (mod->module_flags & MSF_AUTOMATIC_TAGS) safe_printf(" AUTOMATIC TAGS");
    ...
    switch (val->type) {
        ...
        case ATV_TYPE:
            asn1print_expr(val->value.v_type->module->asn1p,
                           val->value.v_type->module, val->value.v_type, flags,
                           0);
            return 0;
        case ATV_INTEGER:
        ...
        case ATV_TRUE:
            ...
            return 0;
        case ATV_TUPLE:
            safe_printf("{%d, %d}", (int)(val->value.v_integer >> 4),
...
static int asn1print_constraint(const asn1p_constraint_t *ct,
                                enum asn1print_flags flags) {
    int symno = 0;
...
static int asn1print_crange_value(const asn1cnst_edge_t *edge, int as_char) {
    switch (edge->type) {
        case ARE_MIN:
            safe_printf("MIN");
            break;
...
static int asn1print_constraint_explain_type(const char *dbg_name,
    ...
    for (i = -1; i < range->el_count; i++) {
        ...
        if (r->left.type != r->right.type || r->left.value != r->right.value) {
            safe_printf("..");
            asn1print_crange_value(&r->right, as_char);
        }
    }
    if (range->extensible) safe_printf(",...");
    safe_printf(type == ACT_EL_RANGE ? ")" : "))");

    if (range->empty_constraint) safe_printf(":Empty!");

    asn1constraint_range_free(range);
    return 0;
}

static int asn1print_constraint_explain(const char *dbg_name,
                                        asn1p_expr_type_e expr_type,
                                        asn1p_constraint_t *ct,
                                        enum cpr_flags cpr) {
    asn1print_constraint_explain_type(dbg_name, expr_type, ct, ACT_EL_RANGE,
                                      cpr);
    safe_printf(" ");
    asn1print_constraint_explain_type(dbg_name, expr_type, ct, ACT_CT_SIZE,
                                      cpr);
    safe_printf(" ");
    asn1print_constraint_explain_type(dbg_name, expr_type, ct, ACT_CT_FROM,
                                      cpr);
...
#define WANT_SPACE()   \
    ...
    if (tc->meta_type != AMT_VALUE && tc->meta_type != AMT_VALUESET &&
        tc->expr_type != A1TC_EXTENSIBLE) {
        if (level) {
            if (tc->Identifier && !(flags & APF_NOINDENT)) safe_printf("\t");
     ...
     */
    if (TQ_FIRST(&(tc->members)) || (tc->expr_type & ASN_CONSTR_MASK) ||
        tc->meta_type == AMT_OBJECT || tc->meta_type == AMT_OBJECTCLASS ||
        ...
        if (put_braces) {
            if (flags & APF_NOINDENT) {
                safe_printf("{");
                if (!TQ_FIRST(&tc->members)) safe_printf("}");
            } else {
                safe_printf(" {");
        ...
        TQ_FOR (se, &(tc->members), next) {
            ...
            } else if ((se->marker.flags & EM_OPTIONAL) == EM_OPTIONAL) {
                safe_printf(" OPTIONAL");
            }
            if (TQ_NEXT(se, next)) {
                safe_printf(",");
                if (!(flags & APF_NOINDENT)) INDENT("\n");
            }
        }

        if (put_braces && TQ_FIRST(&tc->members)) {
            if (!(flags & APF_NOINDENT)) safe_printf("\n");
            INDENT("}");
        }
    }
    ...
    if (!SEQ_OF && tc->constraints) {
        safe_printf(" ");
        if (tc->meta_type == AMT_VALUESET) safe_printf("{");
        asn1print_constraint(tc->constraints, flags);
        if (tc->meta_type == AMT_VALUESET) safe_printf("}");
    }
    ...
    if (tc->meta_type == AMT_VALUE && tc->expr_type != A1TC_EXTENSIBLE) {
     ...
     */
    if (flags & APF_PRINT_CONSTRAINTS && tc->expr_type != A1TC_EXTENSIBLE) {
...
>>>>>>> upstream/vlm_master
    ...
    if (flags & APF_PRINT_CLASS_MATRIX) do {
            ...
            for (ssize_t r = -1; r < (ssize_t)tc->ioc_table->rows; r++) {
                ...
                if (r < 0)
                    ...
                    safe_printf("-- [%*d]", (tc->ioc_table->rows > 9) + 1,
                                r + 1);
                for (col = 0; col < row->columns; col++) {
                    struct asn1p_ioc_cell_s *cell;
                    cell = &row->column[col];
    ...
    if (flags & APF_PRINT_CLASS_MATRIX && tc->lhs_params) do {
            int i;
...
static int asn1print_expr_dtd(asn1p_t *asn, asn1p_module_t *mod,
                              asn1p_expr_t *expr, enum asn1print_flags flags,
                              int level) {
    asn1p_expr_t *se;
