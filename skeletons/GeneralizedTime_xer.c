...
asn_enc_rval_t GeneralizedTime_encode_xer(const asn_TYPE_descriptor_t *td,
    ...
    if (flags & XER_F_CANONICAL) {
        ...
        errno = EPERM;
        if (asn_GT2time_frac((const GeneralizedTime_t *)sptr, &fv, &fd, &tm,
                             1) == -1 &&
            errno != EPERM)
            ASN__ENCODE_FAILED;
        ...
        rv = OCTET_STRING_encode_xer_utf8(td, sptr, ilevel, flags, cb, app_key);
