...
static enum xer_pbd_rval INTEGER__xer_body_decode(
            ...
            case 0x3c: /* '<', start of XML encoded enumeration */
                if (state == ST_LEADSPACE) {
                    ...
                    el = INTEGER_map_enum2value(
                        (const asn_INTEGER_specifics_t *)td->specifics, lstart,
                        lstop);
                    if (el) {
                        ASN_DEBUG("Found \"%s\" => %ld", el->enum_name,
                                  el->nat_value);
    ...
    switch (state) {
        ...
        case ST_DIGITS_TRAILSPACE:
            ...
            switch (asn_strtoimax_lim(dec_value_start, &dec_value_end,
                                      &dec_value)) {
                case ASN_STRTOX_OK:
                    if (specs && specs->field_unsigned &&
                        (uintmax_t)dec_value <= ULONG_MAX) {
                        break;
