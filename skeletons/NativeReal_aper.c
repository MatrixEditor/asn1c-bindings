...
<<<<<<< HEAD
asn_dec_rval_t NativeReal_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
...
<<<<<<< HEAD
    ...
    if (Dbl == NULL) {
        *dbl_ptr = CALLOC(1, sizeof(*Dbl));
        Dbl = (double *)*dbl_ptr;
        if (Dbl == NULL) ASN__DECODE_FAILED;
    }

    memset(&tmp, 0, sizeof(tmp));
    rval = OCTET_STRING_decode_aper(opt_codec_ctx, td, NULL, &ptmp, pd);
    if (rval.code != RC_OK) {
=======
    memset(&tmp, 0, sizeof(tmp));
    rval = OCTET_STRING_decode_aper(opt_codec_ctx, &asn_DEF_REAL, NULL, &ptmp,
                                    pd);
    if(rval.code != RC_OK) {
>>>>>>> upstream/vlm_master
...
<<<<<<< HEAD
asn_enc_rval_t NativeReal_encode_aper(const asn_TYPE_descriptor_t *td,
                                      const asn_per_constraints_t *constraints,
...
<<<<<<< HEAD
    if (asn_double2REAL(&tmp, Dbl)) ASN__ENCODE_FAILED;

    /* Encode a DER REAL */
    erval = OCTET_STRING_encode_aper(td, NULL, &tmp, po);
    if (erval.encoded == -1) erval.structure_ptr = sptr;
=======
