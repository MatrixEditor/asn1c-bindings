 ...
 */
asn_dec_rval_t NativeReal_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    rval = REAL_decode_jer(opt_codec_ctx, td, constraints, (void **)&stp,
                           buf_ptr, size);
    if (rval.code == RC_OK) {
        double d;
...
asn_enc_rval_t NativeReal_encode_jer(const asn_TYPE_descriptor_t *td,
                                     const asn_jer_constraints_t *constraints,
    ...
    if (asn_double2REAL(&tmpreal, native)) ASN__ENCODE_FAILED;
