 ...
 */
asn_enc_rval_t NativeReal_encode_oer(const asn_TYPE_descriptor_t *td,
...
asn_dec_rval_t NativeReal_decode_oer(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    if (constraints && constraints->value.width != 0) {
        ...
        if (size < wire_size) ASN__DECODE_STARVED;
        ...
        switch (wire_size) {
            case sizeof(double): {
                ...
                if (NativeReal__set(td, sptr, tmp) < 0) ASN__DECODE_FAILED;
            } break;
            case sizeof(float): {
                float tmp;
...

    {
        ...
        if (ret) {
            ASN_DEBUG("REAL decoded in %" ASN_PRI_SIZE
