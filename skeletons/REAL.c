...
int REAL_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
                 const void *bptr) {
    const REAL_t *a = aptr;
    const REAL_t *b = bptr;
...
int REAL_copy(const asn_TYPE_descriptor_t *td, void **aptr, const void *bptr) {
    REAL_t *a = *aptr;
    const REAL_t *b = bptr;
...
int asn_REAL2double(const REAL_t *st, double *dbl_value) {
    unsigned int octv;
    ...
    switch (octv & 0xC0) {
        ...
        case 0x00: { /* X.690: 8.5.7 */
            ...
            if (st->buf[st->size] != '\0' || memchr(st->buf, ',', st->size)) {
                ...
                for (p = st->buf + 1, end = st->buf + st->size; p < end;
                     b++, p++)
                    *b = (*p == ',') ? '.' : *p;
                *b = '\0';
            } else {
     ...
     */
    {
        ...
        for (ptr++; ptr < end; ptr++) expval = (expval * 256) + *ptr;

        m = 0.0; /* Initial mantissa value */

        /* Okay, the exponent is here. Now, what about mantissa? */
        end = st->buf + st->size;
        for (; ptr < end; ptr++) m = ldexp(m, 8) + *ptr;

        if (0)
            ASN_DEBUG(
                "m=%.10f, scF=%d, bF=%d, expval=%d, ldexp()=%f, ldexp()=%f\n",
                m, scaleF, baseF, expval, ldexp(m, expval * baseF + scaleF),
 ...
 */
int asn_double2REAL(REAL_t *st, double dbl_value) {
    double test = -0.0;
    ...
    if (mval && !(mval & 1)) {
        ...
        if (!(mval & 0x0f)) /* Speed-up a little */
            shift_count = 4;
        while (((mval >> shift_count) & 1) == 0) shift_count++;
