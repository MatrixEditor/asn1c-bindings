...
static int asn_isnan(double d) { return isnan(d); }
...
static ssize_t REAL__jer_dump(double d, int canonical,
    ...
    do {
        ret = snprintf(
            buf, buflen,
            canonical ? "%.17E" /* Precise */ : "%.15f" /* Pleasant*/, d);
        if (ret < 0) {
    ...
    if (canonical) {
        ...
        enum { LZSTATE_NOTHING, LZSTATE_ZEROES } lz_state = LZSTATE_NOTHING;
        ...
        *dot = 0x2e; /* Replace possible comma */
        ...
        assert(*s == 0x45);
        {
            ...
            for (; expptr <= end; s++, expptr++) *s = *expptr;
        }
    } else {
        /*
...
asn_enc_rval_t REAL_encode_jer(const asn_TYPE_descriptor_t *td,
    ...
    if (!st || !st->buf || asn_REAL2double(st, &d)) ASN__ENCODE_FAILED;
 ...
 */
static enum jer_pbd_rval REAL__jer_body_decode(const asn_TYPE_descriptor_t *td,
                                               void *sptr,
    ...
    if (jerdata[0] == 0x22 /* '"' */) {
        size_t i;
        for (i = 0;
             i < sizeof(specialRealValue_jer) / sizeof(specialRealValue_jer[0]);
             i++) {
            ...
            if (asn_double2REAL(st, dv)) return JPBD_SYSTEM_FAILURE;
    ...
    if (asn_double2REAL(st, value)) return JPBD_SYSTEM_FAILURE;

    return JPBD_BODY_CONSUMED;
}

asn_dec_rval_t REAL_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
