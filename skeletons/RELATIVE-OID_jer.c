...
static enum jer_pbd_rval RELATIVE_OID__jer_body_decode(
    ...
    for (; chunk_end >= p; --chunk_end) {
        if (*chunk_end == CQUOTE) break;
    }
    if (chunk_end - p < 0) return JPBD_BROKEN_ENCODING;
...
asn_dec_rval_t RELATIVE_OID_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
                                       const asn_TYPE_descriptor_t *td,
                                       const asn_jer_constraints_t *constraints,
                                       void **sptr, const void *buf_ptr,
                                       size_t size) {
    return jer_decode_primitive(opt_codec_ctx, td, sptr, sizeof(RELATIVE_OID_t),
                                buf_ptr, size, RELATIVE_OID__jer_body_decode);
}

asn_enc_rval_t RELATIVE_OID_encode_jer(const asn_TYPE_descriptor_t *td,
                                       const asn_jer_constraints_t *constraints,
