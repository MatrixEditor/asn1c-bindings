...
asn_enc_rval_t UTCTime_encode_jer(const asn_TYPE_descriptor_t *td,
    ...
    errno = EPERM;
    if (asn_UT2time((const UTCTime_t *)sptr, &tm, 1) == -1 && errno != EPERM)
        ASN__ENCODE_FAILED;
    ...
    rv = OCTET_STRING_encode_jer_utf8(td, constraints, sptr, ilevel, flags, cb,
                                      app_key);
