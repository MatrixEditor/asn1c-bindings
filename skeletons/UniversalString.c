...
asn_OCTET_STRING_specifics_t asn_SPC_UniversalString_specs = {
    sizeof(UniversalString_t), offsetof(UniversalString_t, _asn_ctx),
...
int UniversalString_constraint(const asn_TYPE_descriptor_t *td,
                               const void *sptr,
...
ssize_t UniversalString__dump(const UniversalString_t *st,
    ...
    for (end -= 3; ch < end; ch += 4) {
        uint32_t wc =
            (ch[0] << 24) | (ch[1] << 16) | (ch[2] << 8) | ch[3]; /* 4 bytes */
