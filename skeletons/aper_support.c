...
ssize_t aper_get_nsnnwn(asn_per_data_t *pd) {
    ...
    if (b == 1) {
        /* other 10.9.3.x cases not handled, it's doubtful we reach them in
         * practice */
...
long aper_get_constrained_whole_number(asn_per_data_t *pd, long lb, long ub) {
    ...
    ASN_DEBUG("aper get constrained_whole_number with lb %ld and ub %ld", lb,
              ub);
...
ssize_t aper_put_length(asn_per_outp_t *po, ssize_t lb, ssize_t ub, size_t n,
    ...
    *need_eom = 0;
    ...
    /* X.691 2002 10.9.3.3 */
    if (constrained && ub < 65536)
        return aper_put_constrained_whole_number(po, lb, ub, n + lb)
                   ? -1
                   : (ssize_t)n;
