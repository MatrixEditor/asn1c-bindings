...
>>>>>>> upstream/vlm_master
    ...
    0,
                                      ...
                                      CHOICE_decode_jer,  CHOICE_encode_jer,
#else
    0,
    0,
...
ber_tlv_tag_t CHOICE_outmost_tag(const asn_TYPE_descriptor_t *td,
    ...
    if (present > 0 && present <= td->elements_count) {
        ...
        if (elm->flags & ATF_POINTER) {
            memb_ptr =
                *(const void *const *)((const char *)ptr + elm->memb_offset);
        } else {
...
int CHOICE_copy(const asn_TYPE_descriptor_t *td, void **aptr,
                const void *bptr) {
    if (!td) return -1;
