 ...
 */
asn_dec_rval_t CHOICE_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
                                 const asn_TYPE_descriptor_t *td,
                                 const asn_jer_constraints_t *constraints,
                                 void **struct_ptr, const void *buf_ptr,
                                 size_t size) {
    /*
    ...
    for (edx = ctx->step; ctx->phase <= 4;) {
        ...
        if (ctx->phase == 2) {
            ...
            unsigned CC_UNUSED(old_present);
            ...
            tmprval = elm->type->op->jer_decoder(
                opt_codec_ctx, elm->type,
                elm->encoding_constraints.jer_constraints, memb_ptr2, buf_ptr,
                size);
            JER_ADVANCE(tmprval.consumed);
            ASN_DEBUG("JER/CHOICE: itdf: [%s] code=%d", elm->type->name,
                      tmprval.code);
            old_present =
        ...
        if (ctx->phase == 4) {
            ASN_DEBUG("skip_unknown(%d, %ld)", scv, (long)ctx->left);
                ...
                for (edx = 0; edx < td->elements_count; edx++) {
                    ...
                    switch (scv) {
                        case JCK_KEY:
                            ...
                            /* skip colon */
                            ch_size = jer_next_token(&ctx->context, buf_ptr,
                                                     size, &ch_type);
                            if (ch_size == -1) {
                                RETURN(RC_FAIL);
                            } else {
...
asn_enc_rval_t CHOICE_encode_jer(const asn_TYPE_descriptor_t *td,
                                 const asn_jer_constraints_t *constraints,
