...
SEQUENCE_OF_encode_jer(const asn_TYPE_descriptor_t *td, const asn_jer_constraints_t *constraints,
    ...
    for(i = 0; i < list->count; i++) {
         ...
         * If as_XMLValueList were incorrectly set for JER, it would produce
         * invalid JSON like: ["tagName"] instead of [] for empty elements,
         * or mixed content like: [value, "tagName"] which violates JSON syntax.
