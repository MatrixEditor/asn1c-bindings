...
asn_dec_rval_t SEQUENCE_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    switch (ctx->phase) {
        case 0:
            ...
            rval = ber_check_tags(opt_codec_ctx, td, ctx, ptr, size, tag_mode,
                                  1, &ctx->left, 0);
            if (rval.code != RC_OK) {
        ...
        case 1:
            ...
            for (edx = ((size_t)ctx->step >> 1); edx < td->elements_count;
                 ...
                 */
                ASN_DEBUG("In %s SEQUENCE left %d, edx=%" ASN_PRI_SIZE
                          " flags=%d"
                          " opt=%d ec=%d",
                ...
                ASN_DEBUG(
                    "Current tag in %s SEQUENCE for element %" ASN_PRI_SIZE
                    " "
                ...
                if (ctx->left < 0 && ((const uint8_t *)ptr)[0] == 0) {
                    if (LEFT < 2) {
                        ...
                        ASN_DEBUG("edx = %" ASN_PRI_SIZE ", opt = %d, ec=%d",
...
>>>>>>> upstream/vlm_master
                 ...
                 */
                if (elements[edx].flags & ATF_OPEN_TYPE) {
                    rval = OPEN_TYPE_ber_get(opt_codec_ctx, td, st,
                                             &elements[edx], ptr, LEFT);
                } else {
                    rval = elements[edx].type->op->ber_decoder(
                        opt_codec_ctx, elements[edx].type, memb_ptr2, ptr, LEFT,
                        elements[edx].tag_mode);
                }
                ASN_DEBUG("In %s SEQUENCE decoded %" ASN_PRI_SIZE
                          " %s of %d "
                          "in %d bytes rval.code %d, size=%d",
...
asn_enc_rval_t SEQUENCE_encode_der(const asn_TYPE_descriptor_t *td,
    ...
    for (edx = 0; edx < td->elements_count; edx++) {
        ...
        if (elm->flags & ATF_POINTER) {
            ...
            if (!*memb_ptr2) {
                ASN_DEBUG("Element %s %" ASN_PRI_SIZE " not present", elm->name,
                          edx);
