    ...
    for (edx = ctx->step; ctx->phase <= 3;) {
        ...
        if (ctx->phase == 2) {
            ...
            if (elm->flags & ATF_OPEN_TYPE) {
                tmprval =
                    OPEN_TYPE_jer_get(opt_codec_ctx, td, st, elm, ptr, size);
            } else {
                /* Invoke the inner type decoder, m.b. multiple times */
                tmprval = elm->type->op->jer_decoder(
                    opt_codec_ctx, elm->type,
                    elm->encoding_constraints.jer_constraints, memb_ptr2, ptr,
                    size);
            }
            ...
            ctx->step = ++edx;
            ASN_DEBUG("JER/SEQUENCE phase => %d, step => %d", ctx->phase,
                      ctx->step);
            /* Fall through */
        }
        ...
        ASN_DEBUG("JER/SEQUENCE: scv = %d, ph=%d [%s]", scv, ctx->phase,
                  td->name);
...
>>>>>>> upstream/vlm_master
            ...
            case JCK_UNKNOWN:
                    ...
                    for (n = edx; n < edx_end; n++) {
                        ...
                        switch (scv) {
                            case JCK_KEY:
                                ...
                                /* skip colon */
                                ch_size = jer_next_token(&ctx->context, ptr,
                                                         size, &ch_type);
                                if (ch_size == -1) {
                                    RETURN(RC_FAIL);
                                } else {
                ...
                if (IN_EXTENSION_GROUP(
                        specs,
                        edx + (edx < td->elements_count ? elements[edx].optional
                                                        : 0))) {
