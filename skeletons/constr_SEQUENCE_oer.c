    ...
    switch (ctx->phase) {
        ...
        case 1: {
            ...
            for (edx = (ctx->step >> 1); IN_ROOT_GROUP_PRED(edx);
                ...
                if (elm->optional) {
                    ...
                    if (present < 0) {
                        ASN_DEBUG("Presence map ended prematurely: %d",
                                  present);
                        RETURN(RC_FAIL);
                    } else if (present == 0) {
            ...
            microphase2_decode_continues:
                if (elm->flags & ATF_OPEN_TYPE) {
                    rval = OPEN_TYPE_oer_get(opt_codec_ctx, td, st, elm, ptr,
                                             size);
                } else {
                    ...
                    rval = elm->type->op->oer_decoder(
                        ...
                        elm->encoding_constraints.oer_constraints, memb_ptr2,
                        ptr, size);
                }
                switch (rval.code) {
                    case RC_OK:
                        ...
                        break;
                    case RC_WMORE:
                        ASN_DEBUG("More bytes needed at element %s \"%s\"",
                                  td->name, elm->name);
                        ADVANCE(rval.consumed);
                        RETURN(RC_WMORE);
                    case RC_FAIL:
                        ASN_DEBUG("Decoding failed at element %s \"%s\"",
                                  td->name, elm->name);
                        RETURN(RC_FAIL);
                }
        ...
        case 2:
            assert(ctx->ptr);
            {
                ...
                preamble->buffer =
                    0; /* Will do extensions_present==1 next time. */
                ...
                extadds =
                    asn_bit_data_new_contiguous(ptr, len * 8 - unused_bits);
                if (!extadds) {
                    RETURN(RC_FAIL);
                }
                FREEMEM(preamble);
            ...
            /* Fall through */
        case 3:
            ASN_DEBUG("OER SEQUENCE %s Decoding PHASE 3 (Extensions)",
                      td->name);
            for (; ctx->step < (signed)td->elements_count; ctx->step++) {
                ...
                switch (asn_get_few_bits(extadds, 1)) {
                    case -1:
                        /*
                         * Not every one of our extensions is known to the
                         * remote side. Continue filling in their defaults
                         * though.
                         */
                        /* Fall through */
                    case 0:
...
>>>>>>> upstream/vlm_master
        ...
        if (has_extensions_bit) {
            for (edx = specs->first_extension; edx < td->elements_count;
                 edx++) {
                asn_TYPE_member_t *elm = &td->elements[edx];
