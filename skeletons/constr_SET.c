    ...
    for (edx = 0; edx < td->elements_count;
        ...
        if ((pres & must) == must) {
            ...
            ASN_DEBUG(
                ...
                "are not present",
                td->name, midx, pres, must, (~(pres & must) & must));
            return 0;
        }
    }
...
void SET_free(const asn_TYPE_descriptor_t *td, void *ptr,
...
int SET_constraint(const asn_TYPE_descriptor_t *td, const void *sptr,
...
int SET_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
                const void *bptr) {
    size_t edx;
...
int SET_copy(const asn_TYPE_descriptor_t *td, void **aptr, const void *bptr) {
    if (!td) return -1;
