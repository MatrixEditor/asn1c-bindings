        ...
        switch (method) {
            ...
            case SOES_CUPER:
                ...
                if (erval.encoded != -1) {
                    ...
                    assert(encoding_el->length ==
                           (size_t)(erval.encoded + 7) / 8);
...
int SET_OF_copy(const asn_TYPE_descriptor_t *td, void **aptr,
                const void *bptr) {
    if (!td) return -1;
    ...
    if (b->size) {
        ...
        for (int i = 0; i < b->count; i++) {
            ...
            if (bmemb) {
                void *amemb = 0;
                int ret;
                ret = td->elements->type->op->copy_struct(td->elements->type,
                                                          &amemb, bmemb);
                if (ret != 0) return ret;
                a->array[i] = amemb;
            } else {
