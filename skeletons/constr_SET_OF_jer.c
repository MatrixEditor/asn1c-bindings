 ...
 */
asn_dec_rval_t SET_OF_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    for (; ctx->phase <= 2;) {
        ...
        if (ctx->phase == 2) {
            ...
            ASN_DEBUG("JER/SET OF element [%s]", (*element->name)
                                                     ? element->name
                                                     : element->type->xml_tag);
            tmprval = element->type->op->jer_decoder(
                opt_codec_ctx, element->type,
                element->encoding_constraints.jer_constraints, &ctx->ptr,
                buf_ptr, size);
...
>>>>>>> upstream/vlm_master
        ...
        ASN_DEBUG("JER/SET OF: scv = %d, ph=%d t=%s", scv, ctx->phase,
                  td->name);
        switch (scv) {
            case JCK_AEND:
...
asn_enc_rval_t SET_OF_encode_jer(const asn_TYPE_descriptor_t *td,
                                 const asn_jer_constraints_t *constraints,
