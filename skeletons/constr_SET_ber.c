 ...
 */
asn_dec_rval_t SET_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    switch (ctx->phase) {
        ...
        case 4: /* Only 00 is expected */
            ASN_DEBUG("SET %s Leftover: %ld, size = %ld", td->name,
                      (long)ctx->left, (long)size);
        ...
        case 5:
            ...
            if (!_SET_is_populated(td, st)) RETURN(RC_FAIL);
 ...
 */
asn_enc_rval_t SET_encode_der(const asn_TYPE_descriptor_t *td, const void *sptr,
