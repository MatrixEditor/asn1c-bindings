 ...
 */
asn_dec_rval_t SET_decode_jer(const asn_codec_ctx_t *opt_codec_ctx,
...
>>>>>>> upstream/vlm_master
    ...
    for (edx = ctx->step; ctx->phase <= 3;) {
        ...
        ASN_DEBUG("JER/SET: scv = %d, ph=%d [%s]", scv, ctx->phase, td->name);
...
>>>>>>> upstream/vlm_master
            ...
            case JCK_KEY:
            case JCK_UNKNOWN:
                ASN_DEBUG("JER/SET: scv=%d, ph=%d, edx=%" ASN_PRI_SIZE "", scv,
                          ctx->phase, edx);
                if (ctx->phase != 1) {
