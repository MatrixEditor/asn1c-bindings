 ...
 */
asn_dec_rval_t SET_decode_xer(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    for (edx = ctx->step; ctx->phase <= 3;) {
        ...
        switch (tcv) {
            ...
            case XCT_UNKNOWN_BO:
                ...
                for (edx = 0; edx < td->elements_count; edx++) {
                    switch (
                        xer_check_tag(buf_ptr, ch_size, elements[edx].name)) {
                        case XCT_BOTH:
                        case XCT_OPENING:
                            /*
                             * Process this member.
        ...
        ASN_DEBUG("Unexpected XML tag in SET, expected \"%s\"", xml_tag);
        break;
    }
...
asn_enc_rval_t SET_encode_xer(const asn_TYPE_descriptor_t *td, const void *sptr,
