...
static int has_codec_defined(const asn_TYPE_descriptor_t *td,
...
>>>>>>> upstream/vlm_master
    ...
    if (isatty(1)) {
        const int is_text_output =
            osyntax == ATS_NONSTANDARD_PLAINTEXT || osyntax == ATS_BASIC_XER ||
            osyntax == ATS_CANONICAL_XER || osyntax == ATS_JER;
        if (is_text_output) {
            binary_out = stdout;
        } else {
        ...
        for (ac_i = (isyntax == ATS_RANDOM) ? -1 : 0; ac_i < ac; ac_i++) {
            ...
            for (first_pdu = 1; (first_pdu || !opt_onepdu); first_pdu = 0) {
                ...
                if (isyntax == ATS_RANDOM) {
                    structure = NULL;
                    if (asn_random_fill(pduType, &structure, random_max_size) !=
                        0) {
...
#endif
                    structure =
                        data_decode_from_file(isyntax, pduType, file, name,
                ...
                if (opt_check) {
                    ...
                    if (asn_check_constraints(pduType, structure, errbuf,
                                              &errlen)) {
                        fprintf(stderr,
                                "%s: ASN.1 constraint "
                                "check failed: %s\n",
                ...
                if (osyntax == ATS_INVALID) {
                    ...
                    erv = asn_encode(NULL, osyntax, pduType, structure,
                                     write_out, binary_out);
                    ...
                    DEBUG("Encoded in %" ASN_PRI_SSIZE " bytes of %s",
                          erv.encoded, ats_simple_name(osyntax));
                }
...
static void buffer_shift_left(size_t offset, int bits) {
    ...
    if (offset) {
        ...
        DEBUG("mriht: %c%c%c%c%c%c%c%c", ((right >> 7) & 1) ? '1' : '0',
...
static void *data_decode_from_file(enum asn_transfer_syntax isyntax,
                                   asn_TYPE_descriptor_t *pduType, FILE *file,
...
<<<<<<< HEAD
                ...
                case ATS_UNALIGNED_CANONICAL_PER:
                    rval =
                        uper_decode(opt_codec_ctx, pduType, (void **)&structure,
                                    i_bptr, i_size, 0, DynamicBuffer.unbits);
                    /* uper_decode() returns bits! */
                    ecbits = rval.consumed %
                             8;          /* Bits consumed from the last byte */
                ...
                case ATS_ALIGNED_CANONICAL_PER:
                    rval =
                        aper_decode(opt_codec_ctx, pduType, (void **)&structure,
                                    i_bptr, i_size, 0, DynamicBuffer.unbits);
...
<<<<<<< HEAD
        switch (rval.code) {
            case RC_OK:
                ...
                DEBUG("RC_OK, finishing up with %ld+%d", (long)rval.consumed,
                      ecbits);
                return structure;
            case RC_WMORE:
                DEBUG(
 ...
 */
static void junk_bytes_with_probability(uint8_t *buf, size_t size,
                                        double prob) {
    static int junkmode;
    uint8_t *ptr;
    uint8_t *end;
