...
static int jer__token_cb(pjson_chunk_type_e type, const void *_chunk_data,
 ...
 */
ssize_t jer_next_token(int *stateContext, const void *buffer, size_t size,
...
jer_check_sym_e jer_check_sym(const void *buf_ptr, int size,
    ...
    if (size < 2 || (buf[0] != CQUOTE || buf[size - 1] != CQUOTE)) {
        if (size >= 2)
            ASN_DEBUG("Broken JSON key: \"%c...%c\"", buf[0], buf[size - 1]);
        return JCK_BROKEN;
    }
    ...
    for (end = buf + size; buf < end; buf++, need_key++) {
        ...
        if (b == 0) return JCK_BROKEN; /* Embedded 0 in buf?! */
    }
    if (*need_key) return JCK_UNKNOWN;
...
asn_dec_rval_t jer_decode_general(
    ...
    int (*opt_unexpected_key_decoder)(void *struct_key, const void *chunk_buf,
                                      size_t chunk_size),
        ...
        if (ch_size == -1) {
            ...
            switch (ch_type) {
                ...
                case PJER_VALUE:
                    ...
                    ADVANCE(ch_size);
                    ADVANCE(jer_whitespace_span(buf_ptr,
                                                size)); /* skip whitespace */
                    ch_size = 1;
                case PJER_KEY:
                case PJER_DLM:
...
int jer_skip_unknown(jer_check_sym_e scv, ber_tlv_len_t *depth) {
    ...
    switch (scv) {
        ...
        case JCK_OEND:
        case JCK_UNKNOWN:
            if (--(*depth) == 0) return (scv == JCK_OEND) ? 2 : 1;
            return 0;
        default:
            return -1;
    }
