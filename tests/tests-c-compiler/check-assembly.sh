...
END_MAKEFILE
    ...
    if [ -n "${OPT_DATA_DIR}" ]; then
        ...
        CHECK_FUZZER="rm -rf corpus && mkdir corpus && cp ${OPT_DATA_DIR}/*.ber ${OPT_DATA_DIR}/*.in corpus/ 2>/dev/null; ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=print_stacktrace=1 ./check-fuzzer -timeout=3 -max_total_time=60 -max_len=512 corpus"
    else
        CHECK_FUZZER="ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=print_stacktrace=1 ./check-fuzzer -timeout=3 -max_total_time=60 -max_len=512"
    fi
cat <<END_MAKEFILE >> "${testdir}/Makefile"
